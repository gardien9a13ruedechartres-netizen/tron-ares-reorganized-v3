...
function fallbackToExternalPlayer(entry) {
  ...
  const base = 'https://player-universel-iframe.pages.dev/?';
...
(function seedDemo() {
  const customOverlays = [
    { title: "CMTV", logo: "/media/cmtv.png", url: "/pages/cmtv.html" },
    { title: "TVI",  logo: "/media/TVI.png", url: "/pages/tvi.html" },
    { title: "TVIR", logo: "/media/tvir.jpg", url: "/pages/tvi-reality.html" },
    { title: "TVIF", logo: "https://upload.wikimedia.org/wikipedia/commons/0/0d/TVI_Fic%C3%A7%C3%A3o_%282017%29.png", url: "/pages/tvi-ficcao.html" },
    { title: "TVII", logo: "https://upload.wikimedia.org/wikipedia/commons/0/08/TVI_Internacional_%282017%29.png", url: "pages/tvii.html" },
    { title: "SIC",  logo: "/media/sic.jpg", url: "/pages/sic.html" },
    { title: "SICN",  logo: "/media/SICN.webp", url: "//popcdn.day/player.php?stream=SIC-NOTICIAS" },
    { title: "CNN",  logo: "/media/cnn.png", url: "/pages/CNN.html" },
    { title: "RTP1", logo: "/media/rtp1.jpg", url: "//popcdn.day/player.php?stream=RTP1" },
    { title: "RTPN", logo: "/media/rtpn.png", url: "https://player-universel-iframe.pages.dev/?https://streaming-live.rtp.pt/livetvhlsDVR/rtpnHDdvr.smil/playlist.m3u8?DVR" },
    { title: "RTPI", logo: "/media/rtpi.jpg", url: "https://player-universel-iframe.pages.dev/?https://streaming-live.rtp.pt/liverepeater/rtpi.smil/playlist.m3u8" },
    { title: "BTV", logo: "/media/btv.svg", url: "//popcdn.day/go.php?stream=BTV1" },
...
/* ===== Ares Stream URL Pro: recognized TMDb poster ===== */
(function(){
  const MEMORY_KEY = 'tronAres.directSubtitleMemory.v1';
  ...
  function readMemoryItem(url){
    ...
    try {
      ...
      const item = bag && typeof bag === 'object' ? bag[key] : null;
      return (item && typeof item === 'object') ? item : null;
    } catch {
      return null;
    }
  ...
  function ensurePosterUi(){
    const box = qs('streamUrlMemoryRecognizedBox');
    if (!box) return null;
    let row = qs('streamUrlMemoryRecognizedRow');
    let posterWrap = qs('streamUrlMemoryRecognizedPosterWrap');
    let poster = qs('streamUrlMemoryRecognizedPoster');
    let info = qs('streamUrlMemoryRecognizedInfo');
    let badge = qs('streamUrlMemoryRecognizedBadge');
    let meta = qs('streamUrlMemoryRecognizedMeta');

    if (!row) {
      badge = badge || document.createElement('div');
      meta = meta || document.createElement('div');
      row = document.createElement('div');
      row.id = 'streamUrlMemoryRecognizedRow';
      row.style.display = 'flex';
      row.style.alignItems = 'stretch';
      row.style.gap = '12px';

      posterWrap = document.createElement('div');
      posterWrap.id = 'streamUrlMemoryRecognizedPosterWrap';
      posterWrap.style.display = 'none';
      posterWrap.style.flex = '0 0 78px';
      posterWrap.style.width = '78px';

      poster = document.createElement('img');
      poster.id = 'streamUrlMemoryRecognizedPoster';
      poster.alt = 'Poster TMDb';
      poster.loading = 'lazy';
      poster.decoding = 'async';
      poster.referrerPolicy = 'no-referrer';
      poster.style.width = '78px';
      poster.style.height = '117px';
      poster.style.objectFit = 'cover';
      poster.style.borderRadius = '14px';
      poster.style.border = '1px solid rgba(255,145,0,.22)';
      poster.style.background = 'rgba(10,14,22,.85)';
      ...
      info.id = 'streamUrlMemoryRecognizedInfo';
    ...
    return { box, row, posterWrap, poster, info, badge, meta };
  }

  async function fetchTmdbDetails(tmdbId, type){
    const id = String(tmdbId || '').trim();
    if (!id) return null;
    const mediaType = String(type || 'movie').trim() || 'movie';
    const cacheKey = mediaType + ':' + id;
    if (DETAILS_CACHE.has(cacheKey)) return DETAILS_CACHE.get(cacheKey);
    try {
      const apiKey = (typeof TMDB_API_KEY !== 'undefined' && TMDB_API_KEY) ? TMDB_API_KEY : '';
      if (!apiKey) return null;
      const langPrimary = (typeof TMDB_LANG_PRIMARY !== 'undefined' && TMDB_LANG_PRIMARY) ? TMDB_LANG_PRIMARY : 'fr-FR';
      const langFallback = (typeof TMDB_LANG_FALLBACK !== 'undefined' && TMDB_LANG_FALLBACK) ? TMDB_LANG_FALLBACK : 'en-US';
      async function one(lang){
        const u = new URL('https://api.themoviedb.org/3/' + encodeURIComponent(mediaType) + '/' + encodeURIComponent(id));
      ...
      let data = null;
      try { data = await one(langPrimary); } catch {}
      if ((!data || !data.poster_path) && langFallback && langFallback !== langPrimary) {
        try {
          const fb = await one(langFallback);
          if (fb) data = Object.assign({}, fb, data || {});
        } catch {}
      }
      DETAILS_CACHE.set(cacheKey, data || null);
      return data || null;
    } catch {
      DETAILS_CACHE.set(cacheKey, null);
      return null;
    }
  }
  ...
  async function resolvePoster(item){
    ...
    const posterPath = String(item.posterPath || item.poster_path || '').trim();
    if (posterPath) return IMG_BASE + posterPath;
    const data = await fetchTmdbDetails(item.tmdbId, item.tmdbType || 'movie');
    const path = String(data?.poster_path || '').trim();
    if (!path) return '';
    return IMG_BASE + path;
  }

  async function updatePoster(){
    const ui = ensurePosterUi();
    if (!ui) return;
    const url = normalizeUrl(qs('streamUrlInput')?.value || '');
    const item = readMemoryItem(url);
    const posterWrap = ui.posterWrap;
    const poster = ui.poster;
    const token = String(Date.now()) + Math.random();
    poster.dataset.req = token;

    if (!url || !item || !item.tmdbId) {
      posterWrap.style.display = 'none';
      poster.removeAttribute('src');
      return;
    }

    const src = await resolvePoster(item);
    if (poster.dataset.req !== token) return;
    if (src) {
      poster.src = src;
  ...
  const origOpen = (typeof openStreamUrlPanel === 'function') ? openStreamUrlPanel : null;
  if (origOpen && !origOpen.__aresPosterWrapped) {
    const wrapped = function(prefillFromQuery = true){
      const res = origOpen.call(this, prefillFromQuery);
      try { ensurePosterUi(); } catch {}
      try { updatePoster(); } catch {}
      return res;
    };
    wrapped.__aresPosterWrapped = true;
    openStreamUrlPanel = wrapped;
  }

  ['streamUrlInput','streamTitleInput'].forEach((id) => {
    const el = qs(id);
    el?.addEventListener('input', () => { try { updatePoster(); } catch {} });
    el?.addEventListener('change', () => { try { updatePoster(); } catch {} });
    el?.addEventListener('blur', () => { try { updatePoster(); } catch {} });
  });
  window.addEventListener('storage', (ev) => {
    if (!ev || ev.key === MEMORY_KEY) {
      try { updatePoster(); } catch {}
    }
  });
  try { ensurePosterUi(); } catch {}
