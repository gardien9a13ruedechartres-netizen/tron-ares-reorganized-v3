...
document.addEventListener("DOMContentLoaded", () => {
    ...
    loader.load(
      ...
      (gltf) => {
        ...
        if (gltf.animations && gltf.animations.length) {
          ...
          const clip =
            gltf.animations.find((a) => a.name.toLowerCase() === "scene") ||
            gltf.animations[0];

          if (clip) {
            primaryAction = mixer.clipAction(clip);
            primaryAction.play();
          }
        }
  ...
  function animate() {
    ...
    if (mixer) {
      ...
      avatarRoot.rotation.y = Math.sin(performance.now() * 0.001) * 0.25;
    }

    if (avatarRoot && !mixer) {
      avatarRoot.position.y = Math.sin(performance.now() * 0.0016) * 0.04;
    }
