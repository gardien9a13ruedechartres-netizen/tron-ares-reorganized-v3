...
function __livewatchEpgNameForEntry(entry) {
  const explicit = String(entry?.livewatchEpgName || entry?.epgName || '').trim();
...
function __formatEpgRange(item) {
  if (!item) return '';
  const start = __formatEpgTime(item.start || item.start_iso);
  const stop = __formatEpgTime(item.stop || item.stop_iso);
  return start && stop ? `${start} - ${stop}` : (start || stop || '');
}

function __formatEpgRemaining(seconds) {
...
async function __openLivewatchEpgNow(entry) {
  ...
  if (!name) return;
  const panel = __renderLivewatchEpgLoading(entry);
  try {
    const url = `${LIVEWATCH_SMART_EPG_NOW_URL}?name=${encodeURIComponent(name)}`;
...
function createChannelElement(entry, index, sourceType, options) {
  ...
  const livewatchEpgName = __livewatchEpgNameForEntry(entry);
  if (livewatchEpgName) li.dataset.livewatchEpgName = livewatchEpgName;
...
function playShakaDashSource(entry, url) {
  if (!window.shaka || typeof window.shaka.Player !== 'function') return false;
...
function exportIframeToJson() {
  const payload = {
    ...
    items: iframeItems.map(it => ({
      ...
      meoCallLetter: it.meoCallLetter || ''
    }))
  };
  if (jsonArea) jsonArea.value = JSON.stringify(payload, null, 2);
...
(() => {
  function applySubsearchFullscreenPosition() {
    try {
      const overlay = document.querySelector('.subsearch-overlay');
      const card = document.querySelector('.subsearch-card');
      if (!overlay || !card) return;

      const fullscreenEl = document.fullscreenElement || document.webkitFullscreenElement || document.msFullscreenElement || null;
  ...
  document.addEventListener('fullscreenchange', applySubsearchFullscreenPosition);
  document.addEventListener('webkitfullscreenchange', applySubsearchFullscreenPosition);

  const observer = new MutationObserver(() => applySubsearchFullscreenPosition());
  observer.observe(document.documentElement, { childList: true, subtree: true });
})();


// --- fix subsearch overlay opening inside fullscreen player ---
(() => {
  function getFsHost() {
    try {
  ...
  function getOverlayHost() {
    ...
    if (fsHost && fsHost instanceof HTMLElement) return fsHost;
    try {
      if (typeof playerContainer !== 'undefined' && playerContainer instanceof HTMLElement) return playerContainer;
    } catch (_) {}
    return document.body || document.documentElement;
  }

  function applySubsearchOverlayLayout() {
    try {
      const overlay = document.querySelector('.subsearch-overlay');
      const card = overlay ? overlay.querySelector('.subsearch-card') : null;
      if (!overlay || !card) return;

      const host = getOverlayHost();
      const inFullscreen = !!getFsHost();
  ...
  const oldOpen = typeof __subtitleOpenOverlay === 'function' ? __subtitleOpenOverlay : null;
  if (oldOpen && !oldOpen.__tronFullscreenFixed) {
    __subtitleOpenOverlay = function patchedSubtitleOpenOverlay(prefillTitle = '') {
      const result = oldOpen.call(this, prefillTitle);
      try { applySubsearchOverlayLayout(); } catch (_) {}
      try { requestAnimationFrame(() => applySubsearchOverlayLayout()); } catch (_) {}
      try { setTimeout(applySubsearchOverlayLayout, 0); } catch (_) {}
      return result;
    };
    __subtitleOpenOverlay.__tronFullscreenFixed = true;
  }

  const oldClose = typeof __subtitleCloseOverlay === 'function' ? __subtitleCloseOverlay : null;
  if (oldClose && !oldClose.__tronFullscreenFixed) {
    __subtitleCloseOverlay = function patchedSubtitleCloseOverlay() {
      const result = oldClose.call(this);
      try { applySubsearchOverlayLayout(); } catch (_) {}
      return result;
    };
    __subtitleCloseOverlay.__tronFullscreenFixed = true;
  }

  document.addEventListener('fullscreenchange', applySubsearchOverlayLayout, true);
  document.addEventListener('webkitfullscreenchange', applySubsearchOverlayLayout, true);
  window.addEventListener('resize', applySubsearchOverlayLayout, { passive: true });

  try {
    const observer = new MutationObserver(() => applySubsearchOverlayLayout());
