...
const CHANNELS = {
  cmtv: {
    label: "CMTV",
    defaultOrder: ["cable", "direct", "basic", "clouding"],
    sources: {
      cable: {
    ...
    manualSources: {
      ...
      amazingtier: {
        // Keep this as an independent HLS source; the html source remains the
        // wrapper with the menuToggle and return button.
        kind: "clouding",
        id: "amazingtier-cmtv",
...
const AMAZINGTIER_HTML_FALLBACKS = {
  RTP1: {
    kind: "iframe",
...
function allSources(channel) {
  ...
  const htmlFallback = AMAZINGTIER_HTML_FALLBACKS[cloudingChannel];
  return {
    ...(channel.sources || {}),
...
async function resolveSource(channelKey, channel, sourceName) {
  ...
  if (source.kind === "clouding") return resolveAmazingTierSource(channelKey, canonicalName, source);
