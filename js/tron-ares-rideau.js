...
// --- Ares Media Engine signature (console) ---
(() => {
  try {
...
// =====================================================
const NEW_ADDITIONS_JSON = 'media/misc/nouveaux_items.json';
...
let __castSelectedSubtitle = null;

function __setCastSelectedSubtitle(sel) {
  __castSelectedSubtitle = sel && typeof sel === 'object' ? { ...sel } : null;
}

function __clearCastSelectedSubtitle() {
  __castSelectedSubtitle = null;
}
...
const playerInner = playerContainer?.querySelector('.player-inner') || null;

class AresPlayerTransitionManager {
  constructor(options = {}) {
    this.host = options.host || null;
    this.video = options.video || null;
    this.iframeOverlay = options.iframeOverlay || null;
    this.iframeEl = options.iframeEl || null;
    this.playerShell = options.playerShell || null;
    this.fxBtn = options.fxBtn || null;
    this.state = 'idle';
    this.currentSource = 'video';
    this.pendingSource = null;
    this.pendingProfile = null;
    this.pendingEntry = null;
    this.pendingEntryKey = '';
    this.activeVideoKey = '';
    this.hasOpenedOnce = false;
    this.armedForOpen = false;
    this.isFxBoost = !!(this.playerShell && this.playerShell.classList.contains('fx-boost-edges'));
    this.isPip = !!(this.playerShell && this.playerShell.classList.contains('pip-mode'));
  ...
  inject() {
    if (!this.host || this.host.querySelector('#aresTransitionLayer')) {
      this.dom = {
        layer: this.host?.querySelector('#aresTransitionLayer') || null,
        title: this.host?.querySelector('[data-transition-title]') || null,
        meta: this.host?.querySelector('[data-transition-meta]') || null,
        loader: this.host?.querySelector('[data-transition-loader]') || null,
      };
      return;
    }

    const wrap = document.createElement('div');
    wrap.id = 'aresTransitionLayer';
    wrap.className = 'ares-transition-layer hidden profile-soft state-idle source-video';
    wrap.setAttribute('aria-hidden', 'true');
    wrap.innerHTML = `
      <div class="ares-transition-backdrop"></div>
      <div class="ares-transition-vignette"></div>
      <div class="ares-transition-panel ares-transition-panel-left"></div>
      <div class="ares-transition-panel ares-transition-panel-right"></div>
      <div class="ares-transition-sheen"></div>
      <div class="ares-transition-center-flash"></div>
      <div class="ares-transition-center-glow"></div>
      <div class="ares-transition-center-core"></div>
      <div class="ares-transition-noise"></div>
      <div class="ares-transition-hud">
        <div class="ares-transition-pill">
          <span class="ares-transition-pill-kicker" data-transition-meta>Initialisation du flux</span>
          <strong class="ares-transition-pill-title" data-transition-title>Lecture premium</strong>
        </div>
      </div>
      <div class="ares-transition-loader hidden" data-transition-loader>
        <span class="ares-transition-loader-ring"></span>
      </div>
    `;
    this.host.appendChild(wrap);
    this.dom = {
      layer: wrap,
      title: wrap.querySelector('[data-transition-title]'),
      meta: wrap.querySelector('[data-transition-meta]'),
      loader: wrap.querySelector('[data-transition-loader]'),
    };
  }

  bindEnvironment() {
    this._handleReducedMotionChange = () => this.syncEnvironment();
    try {
      this.mqReduced = window.matchMedia('(prefers-reduced-motion: reduce)');
      this.isReducedMotion = !!this.mqReduced.matches;
      if (this.mqReduced.addEventListener) this.mqReduced.addEventListener('change', this._handleReducedMotionChange);
      else if (this.mqReduced.addListener) this.mqReduced.addListener(this._handleReducedMotionChange);
    } catch {}

    document.addEventListener('fullscreenchange', () => this.syncEnvironment(), { passive: true });
    window.addEventListener('resize', () => this.syncEnvironment(), { passive: true });
  }
  ...
  syncEnvironment() {
    ...
    this.isPip = !!(this.playerShell && this.playerShell.classList.contains('pip-mode'));
    this.isFullscreen = !!document.fullscreenElement;
    try { this.isReducedMotion = !!(this.mqReduced && this.mqReduced.matches); } catch {}

    if (!this.dom?.layer) return;
    this.dom.layer.classList.toggle('fx-boost', this.isFxBoost);
    this.dom.layer.classList.toggle('is-pip', this.isPip);
    this.dom.layer.classList.toggle('is-fullscreen', this.isFullscreen);
    this.dom.layer.classList.toggle('reduced-motion', this.isReducedMotion);
  }

  sanitizeLabel(value, fallback = '') {
    return (value == null ? fallback : String(value)).trim() || fallback;
  }

  entryKey(entry) {
    return this.sanitizeLabel(entry?.url || entry?.id || entry?.title || entry?.name || entry?.label, '');
  }

  deriveProfile(entry, sourceType = 'video') {
    if (this.isReducedMotion || this.isPip) return 'soft';
    if (sourceType !== 'video') return 'soft';
    const listType = this.sanitizeLabel(entry?.listType, '').toLowerCase();
    const isZapList = ['channels', 'fr', 'favorites'].includes(listType);
    if (!this.hasOpenedOnce) return 'cinema';
    return isZapList ? 'zap' : 'cinema';
  }

  deriveMeta(entry, sourceType = 'video') {
    const listType = this.sanitizeLabel(entry?.listType, '').toLowerCase();
    if (sourceType !== 'video') return 'Overlay iFrame';
    if (!this.hasOpenedOnce) return 'Initialisation du flux';
    if (listType === 'channels') return 'Zapping chaînes';
    if (listType === 'fr') return 'Zapping FR';
    if (listType === 'favorites') return 'Favori actif';
    if (listType === 'films') return 'Lecture cinéma';
    return 'Lecture premium';
  }

  deriveTitle(entry, sourceType = 'video') {
    const title = this.sanitizeLabel(entry?.title || entry?.name || entry?.label || entry?.channelName, 'Lecture premium');
    return sourceType === 'video' ? title : title;
  }

  setHud(entry, sourceType = 'video', customMeta = '') {
    if (!this.dom?.layer) return;
    if (this.dom.title) this.dom.title.textContent = this.deriveTitle(entry, sourceType);
    if (this.dom.meta) this.dom.meta.textContent = customMeta || this.deriveMeta(entry, sourceType);
  }

  planForEntry(entry, sourceType = 'video') {
    this.pendingEntry = entry || null;
    this.pendingSource = sourceType;
    this.pendingEntryKey = this.entryKey(entry);
    this.pendingProfile = this.deriveProfile(entry, sourceType);
    this.setHud(entry, sourceType);
    ...
    const isNewVideo = !this.hasOpenedOnce || !this.activeVideoKey || (this.pendingEntryKey && this.pendingEntryKey !== this.activeVideoKey);
    this.armedForOpen = !!isNewVideo;
  }

  setState(nextState) {
    this.state = nextState;
    if (!this.dom?.layer) return;
    this.dom.layer.dataset.state = nextState;
    this.dom.layer.classList.remove('state-idle', 'state-loading', 'state-playing', 'state-buffering', 'state-overlay');
    this.dom.layer.classList.add(`state-${nextState}`);
  }

  showLoader(show, label = '') {
    if (!this.dom?.loader) return;
    this.dom.loader.classList.toggle('hidden', !show);
    if (label && this.dom.meta) this.dom.meta.textContent = label;
  }

  resetVisualState() {
    if (!this.dom?.layer) return;
    this.dom.layer.classList.remove('hidden', 'opening', 'opened', 'flash', 'closing', 'buffering-visible');
    void this.dom.layer.offsetWidth;
  }
  ...
  hideImmediate() {
    ...
    this.dom.layer.classList.remove('opening', 'opened', 'flash', 'closing', 'buffering-visible', 'source-iframe');
    this.dom.layer.classList.add('hidden', 'source-video');
  }
  ...
  softClose(reason = '') {
    ...
    window.clearTimeout(this.flashTimer);
    window.clearTimeout(this.closeTimer);
    if (reason) this.dom.layer.dataset.lastReason = reason;
    this.dom.layer.classList.remove('opening', 'flash', 'buffering-visible');
    this.dom.layer.classList.add('closing');
    this.closeTimer = window.setTimeout(() => {
      if (!this.dom?.layer) return;
      this.dom.layer.classList.remove('closing', 'opened');
  ...
  onShowVideo() {
    this.currentSource = 'video';
    if (!this.dom?.layer) return;
    this.dom.layer.classList.remove('source-iframe');
    this.dom.layer.classList.add('source-video');

    if (this.armedForOpen) {
      this.setState('loading');
      this.showLoader(true, this.deriveMeta(this.pendingEntry, 'video'));
  ...
  onShowIframe(entry = null) {
    this.currentSource = 'iframe';
    this.pendingSource = 'iframe';
    this.pendingEntry = entry || this.pendingEntry;
    this.armedForOpen = false;
  ...
  onVideoPlaying() {
    if (this.currentSource !== 'video') this.currentSource = 'video';
    if (!this.armedForOpen) {
      this.showLoader(false);
      if (this.dom?.layer) this.dom.layer.classList.remove('buffering-visible');
      this.setState('playing');
      return;
    }
    this.open(this.pendingProfile || 'zap');
  }

  onVideoWaiting() {
    if (this.currentSource !== 'video' || !this.dom?.layer) return;
    this.setState('buffering');
    this.dom.layer.classList.remove('hidden');
    this.dom.layer.classList.add('buffering-visible');
    this.showLoader(true, 'Buffering…');
  }

  onVideoPause() {
    if (this.currentSource !== 'video') return;
    this.showLoader(false);
    if (this.dom?.layer) this.dom.layer.classList.remove('buffering-visible');
    if (this.state !== 'overlay') this.setState('idle');
  }

  onVideoError() {
    this.armedForOpen = false;
    this.showLoader(false);
    this.softClose('error');
  }

  open(profile = 'zap') {
    if (!this.dom?.layer) return;
    this.syncEnvironment();
    const resolvedProfile = this.isReducedMotion || this.isPip ? 'soft' : profile;
    window.clearTimeout(this.openTimer);
    window.clearTimeout(this.flashTimer);
    window.clearTimeout(this.closeTimer);

    this.dom.layer.classList.remove('hidden', 'closing', 'opened', 'profile-cinema', 'profile-zap', 'profile-soft', 'buffering-visible');
    this.dom.layer.classList.add(`profile-${resolvedProfile}`, 'source-video');

    this.resetVisualState();
    this.showLoader(false);
    this.setState('playing');

    if (resolvedProfile === 'cinema') {
      this.dom.layer.classList.add('flash');
      this.flashTimer = window.setTimeout(() => {
        this.dom?.layer?.classList.remove('flash');
      }, 280);
    }

    this.dom.layer.classList.add('opening');
    const duration = resolvedProfile === 'cinema' ? 1450 : resolvedProfile === 'zap' ? 920 : 520;
    this.openTimer = window.setTimeout(() => {
      if (!this.dom?.layer) return;
      this.dom.layer.classList.add('opened');
      this.dom.layer.classList.remove('opening');
      this.dom.layer.classList.add('hidden');
    }, duration);

    if (this.pendingEntryKey) this.activeVideoKey = this.pendingEntryKey;
    this.pendingSource = null;
    this.pendingProfile = null;
    this.armedForOpen = false;
  ...
  replay(profile = '') {
    const nextProfile = profile || this.deriveProfile(this.pendingEntry || currentEntry, 'video');
    this.pendingSource = 'video';
    this.pendingProfile = nextProfile;
    this.armedForOpen = true;
    this.onShowVideo();
    this.open(nextProfile);
  }
}
const playerTransitionManager = new AresPlayerTransitionManager({
  host: playerInner,
  video: videoEl,
  iframeOverlay,
  iframeEl,
  playerShell: playerContainer,
  fxBtn: fxToggleBtn,
});
...
function showRadioOverlayInPlayer() {
  ...
  if (iframe) {
    const url =
      ...
      ((typeof window !== 'undefined' && window.LUNA_URL_OVERRIDE) ? window.LUNA_URL_OVERRIDE : 'pages/luna.html');
...
function buildCastLoadRequest() {
  ...
  // Sous-titres distants (Chromecast exige une URL HTTP(S) vers un WebVTT)
  try {
    const castSub = __castSelectedSubtitle;
    if (castSub && castSub.vttUrl && /^https?:\/\//i.test(String(castSub.vttUrl))) {
      const track = new chrome.cast.media.Track(1, chrome.cast.media.TrackType.TEXT);
      track.trackContentId = String(castSub.vttUrl);
      track.trackContentType = 'text/vtt';
      track.subtype = chrome.cast.media.TextTrackType.SUBTITLES;
      track.name = String(castSub.label || 'Sous-titres');
      track.language = String(castSub.lang || 'und');
    ...
    console.warn('Cast subtitle track inject failed', e);
  }

  const req = new chrome.cast.media.LoadRequest(mediaInfo);
  req.autoplay = true;
  try {
    if (mediaInfo.tracks && mediaInfo.tracks.length) req.activeTrackIds = [1];
  } catch {}
...
function openStreamUrlPanel(prefillFromQuery = true) {
  ...
  const rawUrl = normalizeStreamUrl(streamUrlInput?.value || '');
  const remembered = __subtitleRecallForUrl(rawUrl);
  if (streamTitleInput && remembered && !(streamTitleInput.value || '').trim()) {
    streamTitleInput.value = remembered.queryLabel || remembered.title || '';
  }
...
const __subtitleAutoSearchState = {
  key: '',
  provider: '',
  langs: '',
  data: null,
...
const __DIRECT_SUBTITLE_MEMORY_KEY = 'tronAres.directSubtitleMemory.v1';
const __DIRECT_SUBTITLE_MEMORY_LIMIT = 180;
let __directSubtitleMemory = (() => {
  try {
    const raw = localStorage.getItem(__DIRECT_SUBTITLE_MEMORY_KEY);
...
function __persistDirectSubtitleMemory() {
  try {
    const entries = Object.entries(__directSubtitleMemory || {})
      .sort((a, b) => Number(b?.[1]?.at || 0) - Number(a?.[1]?.at || 0))
      .slice(0, __DIRECT_SUBTITLE_MEMORY_LIMIT);
    const compact = Object.fromEntries(entries);
    __directSubtitleMemory = compact;
    localStorage.setItem(__DIRECT_SUBTITLE_MEMORY_KEY, JSON.stringify(compact));
  } catch {}
}

function __subtitleBuildDirectUrlKey(url) {
  const raw = String(url || '').trim();
  if (!raw) return '';
  try {
    const parsed = new URL(raw, window.location.href);
    const host = String(parsed.host || '').trim().toLowerCase();
    const path = String(parsed.pathname || '').replace(/\/+$/, '').trim().toLowerCase();
    if (!host && !path) return '';
    return host + path;
  } catch {
    return raw
...
function __subtitleRecallForUrl(url) {
  const key = __subtitleBuildDirectUrlKey(url);
  if (!key) return null;
  const item = __directSubtitleMemory[key];
  return (item && typeof item === 'object') ? item : null;
}

function __subtitleRememberForUrl(url, payload = {}) {
  const key = __subtitleBuildDirectUrlKey(url);
  if (!key) return;
  const prev = (__directSubtitleMemory[key] && typeof __directSubtitleMemory[key] === 'object')
    ? __directSubtitleMemory[key]
    : {};

  const next = {
    key,
    url: String(url || '').trim(),
    title: String(payload.title || prev.title || '').trim(),
    queryLabel: String(payload.queryLabel || prev.queryLabel || '').trim(),
    tmdbId: String(payload.tmdbId || prev.tmdbId || '').trim(),
    tmdbType: String(payload.tmdbType || prev.tmdbType || '').trim() || 'movie',
    at: Date.now()
  };

  if (!next.title && next.queryLabel) next.title = next.queryLabel;
  if (!next.queryLabel && next.title) next.queryLabel = next.title;

  __directSubtitleMemory[key] = next;
  __persistDirectSubtitleMemory();
}

function __subtitleForgetForUrl(url) {
  const key = __subtitleBuildDirectUrlKey(url);
  if (!key) return;
  try { delete __directSubtitleMemory[key]; } catch {}
  __persistDirectSubtitleMemory();
}

function __subtitleApplyRememberedIdentityToEntry(entry, options = {}) {
  if (!entry || entry.listType !== 'direct') return null;
  const remembered = __subtitleRecallForUrl(entry.url || '');
  if (!remembered) return null;

  if (!entry.tmdbId && remembered.tmdbId) entry.tmdbId = remembered.tmdbId;
  if (!entry.tmdbType && remembered.tmdbType) entry.tmdbType = remembered.tmdbType;

  const preferredTitle = String(remembered.queryLabel || remembered.title || '').trim();
  if (preferredTitle && (options.forceTitle || !entry.name || entry.name === 'Stream URL')) {
    entry.name = preferredTitle;
  }
  return remembered;
}

function __subtitleRememberCurrentDirectEntry(payload = {}) {
  if (!currentEntry || currentEntry.listType !== 'direct' || !currentEntry.url) return;
  __subtitleRememberForUrl(currentEntry.url, {
    title: payload.title || currentEntry.name || '',
    queryLabel: payload.queryLabel || payload.title || currentEntry.name || '',
    tmdbId: payload.tmdbId || currentEntry.tmdbId || '',
    tmdbType: payload.tmdbType || currentEntry.tmdbType || 'movie'
  });
}

function __subtitleNormalizeSearchLabel(value) {
  return String(value || '')
    .replace(/\.[a-z0-9]{2,5}(?:\?.*)?$/i, ' ')
    .replace(/[._]+/g, ' ')
    .replace(/\b(1080p|720p|2160p|4k|hdr|dv|webrip|web[- ]dl|bluray|brrip|x264|x265|h\.?264|h\.?265|aac|dts|truehd|atmos|multi|vfq?|vostfr|french|english|proper|repack|extended)\b/ig, ' ')
    .replace(/\s+/g, ' ')
    .trim();
}

function __subtitleGuessTitleFromUrl(url) {
  const pickFromPath = (rawPath) => {
    ...
    const scored = segments
      .map((seg) => {
        const base = seg
          .replace(/\.[a-z0-9]{2,5}(?:\?.*)?$/i, ' ')
          .replace(/^\d+(?:[-_]\d+){0,4}[-_]+/i, '')
          .replace(/(index|master|playlist|manifest|video|stream|play|vod|movie|movies|storage\d*|cache|cdn)/ig, ' ')
          .replace(/[-_]+/g, ' ')
          .replace(/\s+/g, ' ')
          .trim();

        if (!base) return { seg, base: '', score: -1 };

        const alphaCount = (base.match(/[a-zà-ÿ]/ig) || []).length;
        const digitCount = (base.match(/\d/g) || []).length;
        let score = 0;
        if (alphaCount >= 4) score += 20;
        if (/(19|20)\d{2}/.test(base)) score += 10;
        if (/\s/.test(base)) score += 6;
        if (/[a-zà-ÿ]/i.test(base) && /\d/.test(base)) score += 4;
        if (/^(index|master|manifest|playlist)$/i.test(base)) score -= 50;
        if (alphaCount < 3) score -= 20;
        score += Math.min(8, alphaCount);
        score -= Math.min(6, Math.floor(digitCount / 3));
        return { seg, base, score };
      })
      .sort((a, b) => b.score - a.score);

    const best = scored.find(item => item.score > 0 && item.base);
    return best ? __subtitleNormalizeSearchLabel(best.base) : '';
  };

  try {
    const parsed = new URL(String(url || ''), window.location.href);
    return pickFromPath(parsed.pathname || '');
  } catch {
    return pickFromPath(String(url || ''));
  }
}

function __subtitleDeriveDirectSearchTitle(url, title) {
  const explicitTitle = __subtitleNormalizeSearchLabel(title);
  if (explicitTitle) return explicitTitle;
  const remembered = __subtitleRecallForUrl(url);
  const rememberedTitle = __subtitleNormalizeSearchLabel(remembered?.queryLabel || remembered?.title || '');
  if (rememberedTitle) return rememberedTitle;
  const guessedFromUrl = __subtitleGuessTitleFromUrl(url);
  if (guessedFromUrl) return guessedFromUrl;
  return '';
}

function __subtitleBuildSearchCacheKey(provider, query, langs, entry) {
  return [
    String(provider || '').trim().toLowerCase(),
    String(query || '').trim().toLowerCase(),
    String(langs || '').trim().toLowerCase(),
    entry && entry.tmdbId ? String(entry.tmdbId).trim() : ''
  ].join('::');
}

async function __subtitleAutoSearchForEntry(entry, forcedTitle = '') {
  try { __subtitleSyncProviderSelectForTab(); } catch {}

  const remembered = entry && entry.listType === 'direct' ? __subtitleApplyRememberedIdentityToEntry(entry) : null;
  ...
  const query = __subtitleNormalizeSearchLabel(
    forcedTitle
      || (remembered && (remembered.queryLabel || remembered.title))
      || __subtitleGuessTitleFromEntry(entry)
      || __subtitleGuessTitleFromUrl(entry && entry.url)
  );

  if (subtitleSearchTitleInput && query) {
    subtitleSearchTitleInput.value = query;
  }

  if (!query && !(entry && entry.tmdbId)) return;

  const cacheKey = __subtitleBuildSearchCacheKey(provider, query, langs, entry);
  const cacheAge = Date.now() - (__subtitleAutoSearchState.at || 0);
  if (__subtitleAutoSearchState.key === cacheKey && __subtitleAutoSearchState.data && cacheAge < 5 * 60 * 1000) {
    return;
  }

  let data = null;
  if (provider === 'opensubtitles') {
    data = await __subtitleProxySearchOpenSubtitles(query, langs, entry);
  } else if (provider === 'subdl') {
    data = await __subdlSearchSubtitles(query, langs, entry);
  } else {
    return;
  }

  __subtitleAutoSearchState.key = cacheKey;
  __subtitleAutoSearchState.provider = provider;
  __subtitleAutoSearchState.langs = langs;
  __subtitleAutoSearchState.data = data;
  __subtitleAutoSearchState.at = Date.now();
  __subtitleAutoSearchState.source = query;

  if (subtitleSearchOverlay && !subtitleSearchOverlay.classList.contains('hidden')) {
    if (provider === 'opensubtitles') {
      __subtitleRenderResultsOpenSubtitles(data, langs);
    } else if (provider === 'subdl') {
      __subdlRenderResults(data, langs);
    }
  } else {
    const count =
      provider === 'opensubtitles'
        ? (((data && data.data) || []).length)
        : (((data && data.subtitles) || []).length);
    if (count > 0) {
      __subtitleSetStatus('Sous-titres trouvés pour : ' + query);
    }
  }
}
...
function playDirectStream(url, title, { updateUrl = true } = {}) {
  ...
  const remembered = __subtitleRecallForUrl(cleanUrl);
  const directTitle = (title && String(title).trim())
    ? String(title).trim()
    : (remembered?.queryLabel || remembered?.title || 'Stream URL');
  ...
  const entry = {
    ...
    isFavorite: false,
    listType: 'direct',
    tmdbId: remembered?.tmdbId ? String(remembered.tmdbId).trim() : '',
    tmdbType: remembered?.tmdbType ? String(remembered.tmdbType).trim() : ''
  };
  ...
  Promise.resolve().then(() => __subtitleAutoSearchForEntry(entry, entry.name)).catch((e) => {
...
// Autorise aussi les flux lancés via Stream URL (listType: 'direct')
function isMovieContext() {
  const lt = currentEntry?.listType;
  return lt === 'channels' || lt === 'direct';
}
...
function renderChannelList() {
  ...
  if (newAdditionsMode) {
    ...
    if (!idxs.length) {
      ...
      if (hasIds) {
        empty.textContent = 'Aucun film trouvé pour les IDs indiqués dans media/misc/nouveaux_items.json';
...
// NOW PLAYING BAR
// =====================================================
let __npLangBadgeEl = null;

function __ensureNowPlayingLangBadge(){
  if(__npLangBadgeEl) return __npLangBadgeEl;
  if(!audioTrackBtn) return null;

  // Inject minimal style once (keeps it lightweight and self-contained)
  if(!document.getElementById('np-lang-badge-style')){
    const st = document.createElement('style');
    st.id = 'np-lang-badge-style';
    st.textContent = `
      .np-lang-badge{
        display:inline-flex;
        align-items:center;
      ...
      .np-lang-badge .tx{font-size:10px;line-height:1;opacity:.95}
      .np-lang-badge .flags{display:inline-flex;align-items:center;gap:4px}
      .np-lang-badge svg{width:16px;height:12px;display:block;border-radius:2px;box-shadow:0 0 0 1px rgba(255,255,255,.12) inset}
    `;
    document.head.appendChild(st);
  }

  const el = document.createElement('span');
  el.className = 'np-lang-badge';
  el.id = 'npLangBadge';
  el.style.marginRight = '8px';

  // Insert BEFORE the audio track button as requested
  audioTrackBtn.parentNode?.insertBefore(el, audioTrackBtn);

  __npLangBadgeEl = el;
  return el;
}

function __splitLangsCSV(v){
  if(!v) return [];
  return String(v).split(',').map(s => s.trim().toLowerCase()).filter(Boolean);
}

function __isFrenchOnlyAudio(entry){
  const a = __splitLangsCSV(entry?.audio);
  return a.length === 1 && a[0] === 'fr';
}

function __isMultiLang(entry){
  const a = __splitLangsCSV(entry?.audio);
  const s = __splitLangsCSV(entry?.subs);
  return a.length >= 2 || s.length >= 2;
}


function __flagSvg(code){
  // Inline SVG flags to avoid emoji font issues in some browsers
  // Supported: 'FR', 'GB'
  const c = String(code || '').toUpperCase();

  if(c === 'FR'){
    return `<svg viewBox="0 0 3 2" aria-hidden="true" focusable="false">
  ...
  // Simplified Union Jack (GB) — clean and lightweight
  if(c === 'GB'){
  ...
  // Fallback: empty flag
  return `<svg viewBox="0 0 3 2" aria-hidden="true" focusable="false">
    <rect width="3" height="2" fill="#555"></rect>
  </svg>`;
}

function __renderNowPlayingLangBadge(entry){
  const el = __ensureNowPlayingLangBadge();
  if(!el) return;
  ...
  if(__isFrenchOnlyAudio(entry)){
    el.style.display = '';
    el.innerHTML = `<span class="flags">${__flagSvg('FR')}</span><span class="tx">français</span>`;
    return;
  }

  if(__isMultiLang(entry)){
    el.style.display = '';
    el.innerHTML = `<span class="flags">${__flagSvg('FR')}${__flagSvg('GB')}</span><span class="tx">MULTI</span>`;
    return;
  }
...
function updateNowPlayingCounter() {
  ...
  // Total GLOBAL (toutes les listes "jouables" existantes)
  // - frChannels  : onglet FR
  // - channels    : onglet Films (inclut les playlists M3U fusionnées/append)
  // - iframeItems : onglet PT / iFrames
  // Note: on n'ajoute pas "favoritesView" au total pour éviter de compter des doublons.
  const totalAll = (frChannels?.length || 0) + (channels?.length || 0) + (iframeItems?.length || 0);

  // Position (reste spécifique à la liste en cours)
  let pos = 0;
...
function showVideo() {
  __clearTrailerReturnCtx();
  try { playerTransitionManager?.onShowVideo(); } catch {}
  overlayMode = false;
...
function showIframe() {
  try { playerTransitionManager?.onShowIframe(currentEntry || null); } catch {}
  overlayMode = true;
  try {
...
function playEntryAsOverlay(entry) {
  ...
  currentEntry = entry;
  try { playerTransitionManager?.planForEntry(entry, 'iframe'); } catch {}
  activePlaybackMode = 'iframe';
...
function fallbackToExternalPlayer(entry) {
  ...
  const playerUrl =
    'https://play-6eh.pages.dev/?url=' +
    encodeURIComponent(entry.url) +
    '&controls=1&autoplay=1';

  if (iframeEl) {
    try { iframeEl.setAttribute('allowfullscreen', ''); } catch {}
    try { iframeEl.allowFullscreen = true; } catch {}
    try {
...
currentEntry = entry;
  ...
  // Lecture vidéo
  try { playerTransitionManager?.planForEntry(entry, 'video'); } catch {}
  showVideo();
  destroyHls();
  destroyDash();
...
// M3U PARSER
// =====================================================
function parseM3U(content, listType = 'channels', defaultGroup = 'Playlist', sourceId = '') {
  ...
  for (const raw of lines) {
    ...
    if (lastInf) {
      ...
      const audioMatch = lastInf.match(/audio="([^"]+)"/i);
      if (audioMatch) audio = String(audioMatch[1]).trim();

      const subsMatch = lastInf.match(/subs="([^"]+)"/i);
      if (subsMatch) subs = String(subsMatch[1]).trim();
    }
...
// =====================================================
// HELPERS — JSON playlist (export Ares)
// =====================================================
function __isJsonLikeText(raw) {
  const s = String(raw || '').trim();
  return s.startsWith('{') || s.startsWith('[');
}

/**
 * Parse un JSON "export Ares" en entrées de playlist.
 * Formats acceptés :
 *  - { type: 'm3u', version: 1, items: [...] }
 *  - { items: [...] } (tolérance)
 *  - [...] (tolérance: tableau direct)
 */
function parsePlaylistJSON(raw, listType, defaultGroup, sourceId) {
  const s = String(raw || '').trim();
  if (!__isJsonLikeText(s)) return null;
  ...
  let declaredListType = null;

  if (Array.isArray(data)) {
    items = data;
  } else if (data && typeof data === 'object') {
    declaredType = data.type || null;
    declaredListType = data.listType || null;
    items = Array.isArray(data.items) ? data.items : null;
  }

  if (!Array.isArray(items)) return null;
  // si le JSON déclare explicitement un type différent, on refuse (ex: type=iframe)
  if (declaredType && declaredType !== 'm3u') return null;

  const lt = declaredListType || listType || 'channels';
  const groupFallback = defaultGroup || (lt === 'fr' ? 'FR' : 'Playlist JSON');

  const results = [];
  items.forEach((item) => {
    const name = (item && item.name) ? String(item.name) : 'Sans titre';
    ...
    results.push({
      ...
      name,
      url,
      logo: normalizeLogo(item && item.logo, name),
      group: (item && item.group) ? String(item.group) : groupFallback,
      isIframe: !!(item && item.isIframe) || isYoutubeUrl(url),
      isFavorite: !!(item && item.isFavorite),
      listType: lt,
      // pour permettre l'auto-refresh stable + catégories
      sourceId: sourceId || ''
    });
  });
  ...
  try {
    if (isProbablyPlaylist(finalUrl)) {
      ...
      if (trimmed.startsWith('#EXTM3U')) {
        const parsed = parseM3U(text, 'channels', 'Playlist', url);
      ...
      } else if (__isJsonLikeText(trimmed)) {
        const parsedJson = parsePlaylistJSON(text, 'channels', 'Playlist JSON', url);
        if (Array.isArray(parsedJson) && parsedJson.length) {
          if (!append) channels.splice(0, channels.length);
          channels.push(...parsedJson);
          renderLists();
          if (autoplay && parsedJson.length && currentIndex === -1) {
            playChannel(channels.length - parsedJson.length);
          }
          if (!silent) setStatus('Playlist JSON chargée (' + parsedJson.length + ' entrées)');
        } else {
          if (!silent) setStatus('JSON playlist invalide');
        }
      } else {
        const entry = {
...
async function loadFrM3u(url, opts = {}) {
  ...
  try {
    ...
    let parsed = null;
    if (trimmed.startsWith('#EXTM3U')) {
      parsed = parseM3U(text, 'fr', 'FR', url);
    } else if (__isJsonLikeText(trimmed)) {
      parsed = parsePlaylistJSON(text, 'fr', 'FR', url);
    }

    if (!Array.isArray(parsed) || !parsed.length) {
      console.error('Fichier FR non valide (ni M3U ni JSON)');
      return;
    }
...
function loadFromFile(file) {
  ...
  if (/\.(m3u8?|json)$/i.test(file.name)) {
    reader.onload = () => {
      const text = String(reader.result || '');
      let parsed = null;
      const trimmed = text.trim();
      if (trimmed.startsWith('#EXTM3U')) {
        parsed = parseM3U(text, 'channels', 'Playlist locale', 'local:' + ((file && file.name) ? file.name : 'file'));
      } else if (__isJsonLikeText(trimmed)) {
        parsed = parsePlaylistJSON(text, 'channels', 'Playlist JSON (local)', 'local:' + ((file && file.name) ? file.name : 'file'));
      }
      if (!Array.isArray(parsed) || !parsed.length) {
        setStatus('Fichier local invalide (ni M3U ni JSON)');
        return;
      }
      channels.push(...parsed);
      renderLists();
...
document.querySelectorAll('.tab-btn').forEach(btn => {
  btn.addEventListener('click', () => {
    ...
    // ✅ Spécial "Films" (channelList) : on joue l'intro en boucle au lieu de lancer le 1er item
    if (currentListType === 'channels') {
      // Si un film est déjà sélectionné, on ne force pas l'intro
      if (typeof currentIndex === 'number' && currentIndex >= 0) {
        // no-op
      } else {
        try { playIntroLoop(); } catch (_) {}
      }
    } else {
...
fullPageBtn?.addEventListener('click', () => {
  ...
  try { playerTransitionManager?.syncEnvironment(); } catch {}
});
...
function __syncRememberedTitleFromStreamUrlInput() {
  const url = normalizeStreamUrl(streamUrlInput?.value || '');
  if (!url || !streamTitleInput) return;
  const remembered = __subtitleRecallForUrl(url);
  if (!remembered) return;
  const preferred = String(remembered.queryLabel || remembered.title || '').trim();
  if (!preferred) return;
  if (!(streamTitleInput.value || '').trim()) {
    streamTitleInput.value = preferred;
    setStatus('Titre mémorisé retrouvé pour ce flux');
  }
}

streamUrlInput?.addEventListener('change', __syncRememberedTitleFromStreamUrlInput);
streamUrlInput?.addEventListener('blur', __syncRememberedTitleFromStreamUrlInput);
...
// Video events
videoEl?.addEventListener('playing', () => { try { playerTransitionManager?.onVideoPlaying(); } catch {} markProgress(); setStatus('Lecture en cours'); });
videoEl?.addEventListener('pause', () => { try { playerTransitionManager?.onVideoPause(); } catch {} setStatus('Pause'); });
videoEl?.addEventListener('waiting', () => { try { playerTransitionManager?.onVideoWaiting(); } catch {} setStatus('Buffering…'); });
videoEl?.addEventListener('error', () => {
  try { playerTransitionManager?.onVideoError(); } catch {}
...
const OS_DEFAULTS = {
  ...
  apiKey: 'SZBnt9EmjgdlxNDUfeAl3A6GNxrdqOGU',

  // Login OpenSubtitles (uniquement nécessaire pour télécharger)
  username: 'vstirious',
  password: '9123143Salema',
  ...
  useProxy: true,
  proxyBase: 'https://opensubtitles-proxy-hytj.onrender.com/os',
};
...
function __osOpenOfflineOverlay(subtitleId, fileId) {
  ...
  applyCachedBtn.addEventListener('click', () => {
    ...
    try {
      ...
      try {
        if (__osLoadConfig().useProxy && fileId) {
          __setCastSelectedSubtitle({
            provider: 'opensubtitles',
            fileId: Number(fileId),
            lang: (meta?.language || 'und'),
            label,
            vttUrl: __osProxyBuildCastVttUrl(fileId, 'srt')
          });
          if (typeof CAST !== 'undefined' && CAST.isConnected) {
            try { castLoadCurrentEntry(true); } catch (e) { console.warn('Cast subtitle reload failed', e); }
          }
        }
      } catch (e) {
        console.warn('Cast subtitle setup failed', e);
      }
...
// Rend le bouton ⚙ "Réglages OpenSubtitles" invisible par défaut.
// Il devient visible seulement si la checkbox #toggleOsSettingsBtnCheckbox est cochée.
function __osApplyGearVisibility(gearBtn) {
  try {
    const cb = document.getElementById('toggleOsSettingsBtnCheckbox');
    const visible = !!(cb && cb.checked);
    if (gearBtn) gearBtn.style.display = visible ? '' : 'none';
  } catch (e) {
    if (gearBtn) gearBtn.style.display = 'none';
  }
}

let __osGearObserver = null;
function __osStartGearObserver() {
  if (__osGearObserver) return;
  const ov = document.getElementById('subtitleSearchOverlay');
  if (!ov || !window.MutationObserver) return;

  __osGearObserver = new MutationObserver(() => {
    const btn = ov.querySelector('button.subsearch-mini-btn[title="Réglages OpenSubtitles"]');
    if (btn) __osApplyGearVisibility(btn);
  });
  __osGearObserver.observe(ov, { attributes: true, childList: true, subtree: true, attributeFilter: ['class'] });
}

function __osBindGearVisibility(gearBtn) {
  try {
    const cb = document.getElementById('toggleOsSettingsBtnCheckbox');
    if (!cb || cb.__osBound) {
      __osApplyGearVisibility(gearBtn);
      return;
    }
    cb.__osBound = true;
    cb.addEventListener('change', () => __osApplyGearVisibility(gearBtn));
    __osApplyGearVisibility(gearBtn);
  } catch (e) {
    __osApplyGearVisibility(gearBtn);
  }
...
function __osEnsureSettingsUi() {
  ...
  // Par défaut, on cache le bouton ⚙ (visible seulement via checkbox discrète)
  __osBindGearVisibility(gear);
const modal = document.createElement('div');
  ...
  const fProxyBase = field('URL base du proxy', "ex: https://opensubtitles-proxy-hytj.onrender.com/os", 'text');
...
const foot = document.createElement('div');
  ...
  const loaderUrlWrap = (() => {
    ...
    const DEFAULT_LOADER_URL = 'https://www.opensubtitles.com';
...
function __subtitleIsFilmsTabActive() {
  const tab = document.querySelector('.tab-btn.active')?.dataset?.tab || currentListType || '';
...
function __subtitleSyncProviderSelectForTab() {
  ...
  if (isFilms) {
    // Films: SubDL visible et disponible
    if (!subOpt) {
      try {
    ...
    // Hors Films: on masque/désactive SubDL et on force OpenSubtitles
    if (subOpt) {
...
function __subtitleEnsureLookupUi() {
  if (!subtitleSearchOverlay || !subtitleSearchTitleInput) return null;
  let wrap = document.getElementById('subtitleLookupUiWrap');
  let actions = document.getElementById('subtitleLookupActions');
  let identifyBtn = document.getElementById('subtitleTitleLookupBtn');
  let useBtn = document.getElementById('subtitleUseIdentifiedBtn');
  let results = document.getElementById('subtitleLookupResults');

  if (!wrap) {
    wrap = document.createElement('div');
    wrap.id = 'subtitleLookupUiWrap';

    actions = document.createElement('div');
    actions.id = 'subtitleLookupActions';
    actions.className = 'subsearch-actions';
    ...
    identifyBtn.id = 'subtitleTitleLookupBtn';
    identifyBtn.className = 'btn btn-ghost';
    identifyBtn.textContent = 'Identifier le film';

    useBtn = document.createElement('button');
    useBtn.type = 'button';
    useBtn.id = 'subtitleUseIdentifiedBtn';
    useBtn.className = 'btn btn-accent';
    useBtn.textContent = 'Sous-titres du titre choisi';
    useBtn.disabled = true;

    actions.append(identifyBtn, useBtn);

    const hint = document.createElement('div');
    hint.className = 'subsearch-hint';
    hint.style.marginTop = '6px';
    hint.textContent = 'Identifie d’abord le bon film via TMDb, puis lance la recherche de sous-titres.';

    results = document.createElement('div');
    results.id = 'subtitleLookupResults';
    results.className = 'subsearch-results';
    results.style.marginTop = '8px';
    results.style.marginBottom = '8px';

    wrap.append(actions, hint, results);

    const titleRow = subtitleSearchTitleInput.closest('.subsearch-row');
    if (titleRow && titleRow.parentNode) titleRow.parentNode.insertBefore(wrap, titleRow.nextSibling);
  }

  if (!identifyBtn.dataset.bound) {
    identifyBtn.dataset.bound = '1';
    identifyBtn.addEventListener('click', () => { __subtitleIdentifyByTitle(); });
  }

  if (!useBtn.dataset.bound) {
    useBtn.dataset.bound = '1';
    useBtn.addEventListener('click', () => { __subtitleApplyIdentifiedAndSearch(); });
  }

  return { wrap, actions, identifyBtn, useBtn, results };
}

function __subtitleResetIdentifyState(options = {}) {
  const keepQuery = !!options.keepQuery;
  if (!keepQuery) __subtitleIdentifyState.query = '';
  __subtitleIdentifyState.results = [];
  __subtitleIdentifyState.selected = null;
  __subtitleIdentifyState.at = 0;

  const ui = __subtitleEnsureLookupUi();
  if (!ui) return;
  if (ui.useBtn) ui.useBtn.disabled = true;
  if (ui.results) ui.results.innerHTML = '';
}

function __subtitleBuildTmdbLookupQuery(rawValue) {
  const cleaned = __subtitleNormalizeSearchLabel(rawValue || '');
  if (cleaned) return cleaned;
  if (currentEntry) {
    const guessed = __subtitleNormalizeSearchLabel(
      __subtitleGuessTitleFromEntry(currentEntry) || __subtitleGuessTitleFromUrl(currentEntry.url || '')
    );
...
async function __tmdbSearchSubtitleCandidatesByText(rawQuery) {
  if (!TMDB_API_KEY) throw new Error('TMDB_API_KEY manquant');
  const normalized = __subtitleNormalizeSearchLabel(rawQuery || '');
  const parsed = __extractTitleYear(normalized || '');
  const query = (parsed.title || normalized || '').trim();
  ...
  let search = await __tmdbJson(buildUrl(TMDB_LANG_PRIMARY));
  let results = Array.isArray(search?.results) ? search.results : [];
  if (!results.length && TMDB_LANG_FALLBACK && TMDB_LANG_FALLBACK !== TMDB_LANG_PRIMARY) {
    search = await __tmdbJson(buildUrl(TMDB_LANG_FALLBACK));
    results = Array.isArray(search?.results) ? search.results : [];
  }
  ...
  return results.slice(0, 8).map((item) => {
    ...
    return {
      ...
      language: String(item?.original_language || '').trim(),
      voteAverage: Number(item?.vote_average || 0),
      queryLabel: [title, releaseYear].filter(Boolean).join(' ').trim()
    };
  }).filter(item => item.id && item.title);
}

function __subtitleRenderIdentifyResults(results, rawQuery) {
  const ui = __subtitleEnsureLookupUi();
  if (!ui || !ui.results) return;
  ui.results.innerHTML = '';
  if (ui.useBtn) ui.useBtn.disabled = !(__subtitleIdentifyState.selected && __subtitleIdentifyState.selected.id);

  if (!Array.isArray(results) || !results.length) {
    __subtitleSetStatus('Aucun film TMDb trouvé pour : ' + rawQuery);
    return;
  }
  ...
  results.forEach((item) => {
    const card = document.createElement('div');
    card.className = 'subsearch-result-card';
    ...
    title.textContent = [item.title, item.year ? '(' + item.year + ')' : ''].filter(Boolean).join(' ');

    const meta = document.createElement('div');
    meta.className = 'subsearch-result-meta';
    const metaParts = [];
    if (item.originalTitle && item.originalTitle.toLowerCase() !== item.title.toLowerCase()) metaParts.push(item.originalTitle);
    ...
    left.append(title);
    if (meta.textContent) left.append(meta);

    if (item.overview) {
      const overview = document.createElement('div');
      overview.className = 'subsearch-result-meta';
      overview.style.whiteSpace = 'normal';
      overview.style.lineHeight = '1.35';
      overview.textContent = item.overview.length > 260 ? (item.overview.slice(0, 257) + '…') : item.overview;
      left.append(overview);
    }

    const action = document.createElement('button');
    action.type = 'button';
    action.className = 'subsearch-mini-btn';
    action.textContent = (__subtitleIdentifyState.selected && __subtitleIdentifyState.selected.id === item.id) ? 'Sélectionné' : 'Choisir';
    if (__subtitleIdentifyState.selected && __subtitleIdentifyState.selected.id === item.id) action.disabled = true;
    action.addEventListener('click', () => {
      __subtitleIdentifyState.selected = item;
      __subtitleIdentifyState.query = rawQuery;
      __subtitleIdentifyState.at = Date.now();
      if (subtitleSearchTitleInput) subtitleSearchTitleInput.value = item.queryLabel || item.title;
      if (currentEntry) {
        currentEntry.tmdbId = item.id;
        currentEntry.tmdbType = item.tmdbType || 'movie';
        if (currentEntry.listType === 'direct') {
          currentEntry.name = item.queryLabel || item.title || currentEntry.name;
          __subtitleRememberCurrentDirectEntry({
            title: item.title || item.queryLabel || currentEntry.name,
            queryLabel: item.queryLabel || item.title || currentEntry.name,
            tmdbId: item.id,
            tmdbType: item.tmdbType || 'movie'
          });
        }
      }
      __subtitleRenderIdentifyResults(__subtitleIdentifyState.results, rawQuery);
      __subtitleSetStatus('Film identifié : ' + (item.queryLabel || item.title));
    });

    top.append(left, action);
    card.append(top);
    frag.appendChild(card);
  });

  ui.results.appendChild(frag);
  __subtitleSetStatus('Choisis le bon film, puis lance la recherche de sous-titres.');
}

async function __subtitleIdentifyByTitle() {
  const ui = __subtitleEnsureLookupUi();
  const query = __subtitleBuildTmdbLookupQuery(subtitleSearchTitleInput?.value || '');
  if (!query) {
    __subtitleSetStatus('Entre un titre pour identifier le film.');
    return;
  }
  ...
  try {
    if (ui?.results) ui.results.innerHTML = '';
    if (ui?.useBtn) ui.useBtn.disabled = true;
    __subtitleSetStatus('Identification TMDb…⌛');
    const results = await __tmdbSearchSubtitleCandidatesByText(query);
    __subtitleIdentifyState.query = query;
    __subtitleIdentifyState.results = results;
    __subtitleIdentifyState.selected = null;
    __subtitleIdentifyState.at = Date.now();

    if (results.length === 1) {
      __subtitleIdentifyState.selected = results[0];
      if (subtitleSearchTitleInput) subtitleSearchTitleInput.value = results[0].queryLabel || results[0].title;
      if (currentEntry) {
        currentEntry.tmdbId = results[0].id;
        currentEntry.tmdbType = results[0].tmdbType || 'movie';
        if (currentEntry.listType === 'direct') {
          currentEntry.name = results[0].queryLabel || results[0].title || currentEntry.name;
          __subtitleRememberCurrentDirectEntry({
            title: results[0].title || results[0].queryLabel || currentEntry.name,
            queryLabel: results[0].queryLabel || results[0].title || currentEntry.name,
            tmdbId: results[0].id,
            tmdbType: results[0].tmdbType || 'movie'
          });
        }
      }
    }

    __subtitleRenderIdentifyResults(results, query);
  } catch (e) {
    console.error(e);
    __subtitleSetStatus('Erreur TMDb : ' + (e && e.message ? e.message : String(e)));
  }
}

async function __subtitleApplyIdentifiedAndSearch() {
  const selected = __subtitleIdentifyState.selected;
  if (!selected) {
    __subtitleSetStatus('Choisis d’abord un film identifié.');
    return;
  }

  if (subtitleSearchTitleInput) subtitleSearchTitleInput.value = selected.queryLabel || selected.title;
  if (currentEntry) {
    currentEntry.tmdbId = selected.id;
    currentEntry.tmdbType = selected.tmdbType || 'movie';
    if (currentEntry.listType === 'direct') {
      currentEntry.name = selected.queryLabel || selected.title || currentEntry.name;
      __subtitleRememberCurrentDirectEntry({
        title: selected.title || selected.queryLabel || currentEntry.name,
        queryLabel: selected.queryLabel || selected.title || currentEntry.name,
        tmdbId: selected.id,
        tmdbType: selected.tmdbType || 'movie'
      });
    }
  }
  await __subtitleDoSearch();
}

function __subtitleOpenOverlay(prefillTitle = '') {
  try { __osEnsureSettingsUi(); } catch {}
  try { __subtitleSyncProviderSelectForTab(); } catch {}
  try { __subtitleEnsureLookupUi(); } catch {}
  if (currentEntry && currentEntry.listType === 'direct') {
    __subtitleApplyRememberedIdentityToEntry(currentEntry, { forceTitle: !prefillTitle });
  }
  ...
  const guessedTitle = __subtitleBuildTmdbLookupQuery(prefillTitle || '');
  const provider = subtitleSearchProviderSelect ? subtitleSearchProviderSelect.value : 'opensubtitles';
  const langs = (subtitleSearchLangInput?.value || '').trim();
  const normalizedTitle = __subtitleNormalizeSearchLabel(guessedTitle || '');
  const cacheKey = __subtitleBuildSearchCacheKey(provider, normalizedTitle, langs, currentEntry);

  if (subtitleSearchTitleInput) {
    subtitleSearchTitleInput.value = normalizedTitle;
    try { subtitleSearchTitleInput.focus(); } catch {}
  }
  __subtitleResetIdentifyState({ keepQuery: true });

  if (
    normalizedTitle &&
    __subtitleAutoSearchState.key === cacheKey &&
    __subtitleAutoSearchState.data
  ) {
    if (provider === 'opensubtitles') {
      __subtitleRenderResultsOpenSubtitles(__subtitleAutoSearchState.data, langs);
    } else if (provider === 'subdl') {
      __subdlRenderResults(__subtitleAutoSearchState.data, langs);
    }
    __subtitleSetStatus('Résultats en cache pour : ' + normalizedTitle);
    return;
  }
...
function __subtitleAddTrackFromVttText(vttText, label = 'Sous-titres', lang = 'und') {
  ...
  // Un track blob local n'est pas castable sur Chromecast.
  __clearCastSelectedSubtitle();
...
function __osProxyBuildCastVttUrl(fileId, subFormat = 'srt') {
  const cfg = __osLoadConfig();
  const base = (cfg.proxyBase || '').replace(/\/+$/, '');
  if (!base) throw new Error('Proxy activé mais URL base vide. Ouvre les réglages (⚙).');
  const url = new URL(base + '/api/v1/cast-vtt');
  url.searchParams.set('file_id', String(Number(fileId)));
  url.searchParams.set('sub_format', String(subFormat || 'srt'));
  return url.toString();
}
...
applyBtn.addEventListener('click', async () => {
      ...
      try {
        ...
        const castLabel = (lang ? ('OS ' + lang) : 'OpenSubtitles');
        const vtt = __subtitleSrtToVtt(srtText);
        __subtitleAddTrackFromVttText(vtt, castLabel, (lang || 'und'));
        try {
          if (__osLoadConfig().useProxy) {
            __setCastSelectedSubtitle({
              provider: 'opensubtitles',
              fileId: Number(fileId),
              lang: (lang || 'und'),
            ...
            if (typeof CAST !== 'undefined' && CAST.isConnected) {
              try { await castLoadCurrentEntry(true); } catch (e) { console.warn('Cast subtitle reload failed', e); }
            }
          }
        } catch (e) {
          console.warn('Cast subtitle setup failed', e);
...
// =====================================================
// SubDL (API) — recherche + download (ZIP) + extraction via JSZip
// =====================================================
...
function __subdlSetApiKey(k) {
  try { localStorage.setItem('tronAresSubdlApiKey', String(k || '').trim()); } catch {}
}

function __subdlEnsureApiKey() {
  let k = __subdlGetApiKey();
  if (k) return k;
  const v = prompt('Clé API SubDL manquante.\nColle ta clé SubDL (API Key) :');
  if (v && String(v).trim()) {
    __subdlSetApiKey(String(v).trim());
...
function __subdlNormalizeLanguagesCsv(langsCsv) {
  ...
  if (!raw) return '';
  // SubDL attend plutôt des codes "EN,FR" (doc). On garde aussi les "pt-br" -> "PT-BR".
  return raw
    .split(',')
    .map(s => s.trim())
    .filter(Boolean)
...
function __subdlGuessTypeFromEntry(entry) {
  ...
  // fallback: si SxxEyy détecté dans le titre, on suppose tv
  const q = (subtitleSearchTitleInput?.value || '').toUpperCase();
  if (/\bS\d{1,2}E\d{1,2}\b/.test(q)) return 'tv';
  return 'movie';
}
...
async function __subdlSearchSubtitles(query, languagesCsv, entry) {
  const apiKey = __subdlEnsureApiKey();
  if (!apiKey) throw new Error('Clé SubDL absente.');
  const params = new URLSearchParams();
  ...
  const type = __subdlGuessTypeFromEntry(entry);
  if (type) params.set('type', type);

  // Priorité: TMDb ID si présent (plus fiable)
  if (entry && entry.tmdbId) {
    params.set('tmdb_id', String(entry.tmdbId));
  } else {
    // Sinon on tente film_name (texte)
    if (query) params.set('film_name', String(query));
  }

  // Si c'est une série et qu'on a SxxEyy dans le champ titre, on l'utilise
  if (type === 'tv') {
  ...
  if (!res.ok || !json || json.status !== true) {
    ...
    throw new Error('SubDL (' + res.status + '): ' + (errMsg || 'erreur'));
  }
  return json;
}

function __subdlGetSubtitleLink(sub) {
  if (!sub || typeof sub !== 'object') return '';
  // La doc indique un lien de forme:
  // https://dl.subdl.com/subtitle/3197651-3213944.zip
  const candidates = [
    sub.link, sub.url, sub.download, sub.download_url, sub.dl, sub.dl_url,
    sub.subtitle_link, sub.subtitle_url, sub.zip, sub.zip_url,
    sub.path
  ].filter(Boolean).map(String);

  // Si l'objet contient un id(s), on reconstruit
  const idA = sub.id || sub.sub_id || sub.subtitle_id || sub.sd_subtitle_id || null;
  const idB = sub.file_id || sub.release_id || sub.sub_file_id || null;

  for (const c of candidates) {
    if (/^https?:\/\//i.test(c)) return c;
    if (c.startsWith('/')) return __SUBDL_DL_BASE.replace(/\/+$/,'') + c;
    if (c.includes('subtitle/')) {
      return __SUBDL_DL_BASE.replace(/\/+$/,'') + (c.startsWith('subtitle/') ? '/' + c : '/' + c.replace(/^\/+/, ''));
    }
  }

  // cas: doc "subtitle/{a}-{b}.zip"
...
// --- Cache local SubDL (évite de reconsommer le quota + re-zip) ---
function __subdlCacheKey(link) {
  return 'subdl:' + String(link || '').trim();
}
function __subdlGetCachedVtt(link) {
  try {
    const k = __subdlCacheKey(link);
    const raw = localStorage.getItem(k);
    if (!raw) return null;
    const obj = JSON.parse(raw);
    if (!obj || !obj.vtt) return null;
    // expire après 30 jours (best effort)
    const at = obj.at ? Number(obj.at) : 0;
...
function __subdlPutCachedVtt(link, payload) {
  try {
    const k = __subdlCacheKey(link);
...
async function __subdlFetchZipAndExtractText(downloadUrl) {
  // Télécharge le ZIP et renvoie { text, ext, fileName }
  const res = await fetch(downloadUrl, { method: 'GET' });
  if (!res.ok) throw new Error('Téléchargement ZIP (' + res.status + ')');
  const ab = await res.arrayBuffer();

  // Extraction ZIP via JSZip (chargé côté HTML)
  if (typeof window.JSZip === 'undefined') {
    throw new Error('JSZip manquant (nécessaire pour ouvrir les ZIP SubDL).');
  }

  const zip = await window.JSZip.loadAsync(ab);
  // On cherche en priorité .srt / .vtt
  const files = Object.keys(zip.files || {});
  const pick = (ext) => files.find(f => f.toLowerCase().endsWith(ext));
  const chosen =
    pick('.srt') ||
    pick('.vtt') ||
    (files.length ? files[0] : null);

  if (!chosen) throw new Error('ZIP vide.');
  const fileObj = zip.files[chosen];
  const text = await fileObj.async('string');

  const lower = chosen.toLowerCase();
  const ext = lower.endsWith('.vtt') ? 'vtt' : (lower.endsWith('.srt') ? 'srt' : 'txt');
  const fileName = chosen.split('/').pop() || ('subtitle.' + ext);
  return { text, ext, fileName };
}

function __subdlRenderResults(data, languagesCsv) {
  if (!subtitleSearchResults) return;
  subtitleSearchResults.innerHTML = '';

  const subs = (data && Array.isArray(data.subtitles)) ? data.subtitles : [];
  if (!subs.length) {
    __subtitleSetStatus('Aucun résultat.');
    return;
  }

  __subtitleSetStatus(subs.length + ' résultat(s).');

  subs.slice(0, 30).forEach((sub) => {
    const lang = (sub.language || sub.lang || sub.iso || sub.iso_code || '').toString();
    const release = (sub.release || sub.release_name || sub.version || sub.title || '').toString();
    const name = (sub.name || sub.movie_name || sub.film_name || sub.series_name || '').toString();
    const author = (sub.author || sub.uploader || sub.username || '').toString();
    const hi = (sub.hi != null ? !!sub.hi : (sub.hearing_impaired != null ? !!sub.hearing_impaired : null));
    const dlCount = (sub.downloads != null ? sub.downloads : (sub.download_count != null ? sub.download_count : null));

    const link = __subdlGetSubtitleLink(sub);
    const displayTitle = (release || name || 'Sous-titres SubDL');
    ...
    const metaParts = [];
    if (lang) metaParts.push('Lang: ' + lang);
    if (hi === true) metaParts.push('HI');
    if (author) metaParts.push('By: ' + author);
    if (dlCount != null) metaParts.push('DL: ' + dlCount);
    if (name && release && name !== release) metaParts.push(name);
    left.append(t, meta);
    meta.textContent = metaParts.join(' • ');
    ...
    applyBtn.addEventListener('click', async () => {
      ...
      try {
        ...
        const cached = __subdlGetCachedVtt(link);
        let vttText = cached && cached.vtt ? String(cached.vtt) : null;
        let fileName = cached && cached.fileName ? String(cached.fileName) : '';

        if (!vttText) {
          __subtitleSetStatus('Téléchargement…');
          const dlUrl = link;
          const extracted = await __subdlFetchZipAndExtractText(dlUrl);
          fileName = extracted.fileName || fileName;

          if (extracted.ext === 'vtt') {
            vttText = extracted.text;
          } else {
            vttText = __subtitleSrtToVtt(extracted.text);
          }

          __subdlPutCachedVtt(link, { vtt: vttText, fileName, language: lang || '', title: displayTitle });
        } else {
          __subtitleSetStatus('Depuis le cache…');
        }

        __subtitleAddTrackFromVttText(vttText, (lang ? ('SubDL ' + lang) : 'SubDL'), (lang || 'und'));
        __subtitleSetStatus(cached ? 'Sous-titres appliqués (cache).' : 'Sous-titres appliqués.');
        __subtitleCloseOverlay();
      } catch (e) {
        console.error(e);
    ...
    dlBtn.addEventListener('click', () => {
      if (!link) return;
      // téléchargement natif du ZIP (plus simple)
      window.open(link, '_blank', 'noopener');
      __subtitleSetStatus('Téléchargement lancé (ZIP).');
    });
...
async function __subtitleDoSearch() {
  const provider = subtitleSearchProviderSelect ? subtitleSearchProviderSelect.value : 'opensubtitles';
  const selected = __subtitleIdentifyState.selected;
  const q = (subtitleSearchTitleInput?.value || '').trim() || (selected ? (selected.queryLabel || selected.title) : '');
  const langs = (subtitleSearchLangInput?.value || '').trim();
  ...
  try {
    ...
    if (provider === 'opensubtitles') {
      ...
      const data = await __subdlSearchSubtitles(q, langs, currentEntry);
      __subdlRenderResults(data, langs);
    } else {
    ...
    if (currentEntry && currentEntry.listType === 'direct') {
      currentEntry.name = q || currentEntry.name;
      __subtitleRememberCurrentDirectEntry({
        title: q || currentEntry.name,
        queryLabel: q || currentEntry.name,
        tmdbId: currentEntry.tmdbId || '',
        tmdbType: currentEntry.tmdbType || 'movie'
      });
    }
  } catch (e) {
    console.error(e);
...
(function __bindSubtitleUiOnce(){
  ...
  // Providers: SubDL uniquement sur l’onglet Films
  try { __subtitleSyncProviderSelectForTab(); } catch {}
  try { __subtitleEnsureLookupUi(); } catch {}
  ...
  subtitleSearchTitleInput?.addEventListener('input', () => {
    const live = __subtitleNormalizeSearchLabel(subtitleSearchTitleInput?.value || '');
    const selected = __subtitleIdentifyState.selected;
    if (selected) {
      const selectedLabel = __subtitleNormalizeSearchLabel(selected.queryLabel || selected.title || '');
      if (live && selectedLabel && live !== selectedLabel) {
        __subtitleResetIdentifyState({ keepQuery: true });
      }
    }
  });
...
// =====================================================
// CHARGEMENT DES CHAINES VIA JSON STRUCTURÉ
// =====================================================

(async function loadOverlayChannels() {

  const JSON_SOURCE = "/media/misc/chaines-pt.json";

  try {

    const res = await fetch(JSON_SOURCE, { cache: "no-store" });

    if (!res.ok) {
      throw new Error("Impossible de charger chaines-pt.json");
    }
    ...
    console.log("[TRON] JSON chargé :", channels.length, "chaines");
...
const MAIN_PLAYLIST_URL = "media/misc/playlist.-enriched-tmdb.m3u";
const FR_PLAYLIST_URL   = "media/misc/chaines-fr.json";
// ➕ Playlist films additionnelle (catégorie extra dans la vitrine)
// Mets ici un fichier local (même dossier que index.html) ou une URL https.
const EXTRA_PLAYLIST_URLS = [
  "media/misc/studio_ghibli_tmdb.m3u",
  "media/misc/Disney-Pixar_tmdb.m3u",
  "media/misc/dreamwork_tmdb.m3u",
  "media/misc/Superhero_tmdb.m3u",
  "media/misc/Ancien-Film_tmdb.m3u"
];

// Charge toutes les playlists "extra" en mode append, sans casser le chargement global si une échoue
function loadExtraPlaylists(options){
  return EXTRA_PLAYLIST_URLS.map((url) =>
    loadFromUrl(url, options).catch((err) => {
      console.warn("[Extra playlist] échec:", url, err);
      return null;
    })
  );
}

// 🎬 Intro (loop) — démarre au lancement, sans apparaître dans les listes
const INTRO_LOOP_URL = "media/media/intro-tron.mp4";
let __introIsPlaying = false;
let __introPrevMuted = null;

function __stopIntroIfNeeded() {
  if (!__introIsPlaying) return;
  __introIsPlaying = false;

  try {
    if (videoEl) {
      // on remet un état "normal" pour les contenus suivants
      videoEl.loop = false;
      if (__introPrevMuted !== null) {
        videoEl.muted = __introPrevMuted;
      }
    }
  } catch {}

  __introPrevMuted = null;
}

function playIntroLoop() {
  if (!videoEl) return;

  // Reset sélection (aucune chaîne active)
  currentEntry = null;
  currentIndex = -1;
  currentFrIndex = -1;
  ...
  // Nettoyage players HLS/DASH
  destroyHls();
  destroyDash();

  // Prépare et lance la vidéo intro
  try { videoEl.pause(); } catch {}
  try { videoEl.removeAttribute('src'); } catch {}
  try { videoEl.load(); } catch {}

  __introPrevMuted = (typeof videoEl.muted === 'boolean') ? videoEl.muted : null;
  // Pour maximiser les chances d’autoplay navigateur
  try { videoEl.muted = true; } catch {}

  videoEl.loop = true;
  videoEl.src = INTRO_LOOP_URL;

  __introIsPlaying = true;

  try { videoEl.play(); } catch {}

  updateNowPlaying(null, 'INTRO');
  setStatus('Intro');
  refreshActiveListsUI();
  if (favoriteListEl?.classList.contains('active')) renderFavoritesList();
}
...
async function refreshPlaylistsSilently() {
  ...
  // Snapshot pour fallback si un des fetch échoue
  const prevChannels = channels.slice();
  const prevFr = frChannels.slice();

  // Helper: charge et parse une playlist M3U (sans modifier les tableaux globaux)
  async function __fetchAndParseM3U(url, listType, defaultGroup, cacheBust) {
    if (!url) return null;
    const finalUrl = cacheBust ? (url + (url.includes('?') ? '&' : '?') + '_=' + Date.now()) : url;
    const res = await fetch(finalUrl, { cache: 'no-store' });
    const text = await res.text();
    ...
    // M3U classique
    if (trimmed.startsWith('#EXTM3U')) {
      // ⚠️ sourceId = url (sans cacheBust) pour matcher les entrées entre refresh
      return parseM3U(text, listType, defaultGroup, url);
    }
    // JSON export Ares (type=m3u)
    if (__isJsonLikeText(trimmed)) {
      return parsePlaylistJSON(text, listType, defaultGroup, url);
    }
    return null;
  }
  ...
  try {
    ...
    // 1) On charge la playlist principale
    let mainParsed = null;
    try {
      mainParsed = await __fetchAndParseM3U(MAIN_PLAYLIST_URL, 'channels', 'Playlist', true);
    } catch (e) {
      console.warn('[Auto-refresh] échec main playlist', e);
    }

    // 2) On charge les playlists extras (indépendantes)
    const extrasResults = await Promise.all(
      EXTRA_PLAYLIST_URLS.map(async (u) => {
        try {
          return await __fetchAndParseM3U(u, 'channels', 'Playlist', true);
        } catch (e) {
          console.warn('[Auto-refresh] échec extra', u, e);
          return null;
        }
      })
    );
    ...
    let frParsed = null;
    try {
      frParsed = await __fetchAndParseM3U(FR_PLAYLIST_URL, 'fr', 'FR', true);
    } catch (e) {
      console.warn('[Auto-refresh] échec FR', e);
    }

    // 4) Reconstruction "stable" :
    // - si main échoue => on garde la liste précédente (évite de perdre les catégories)
    // - si une extra échoue => on réinjecte ses entrées depuis l'ancien channels[] grâce à sourceId
    const nextChannels = [];
    if (Array.isArray(mainParsed) && mainParsed.length) {
      nextChannels.push(...mainParsed);
    } else {
      nextChannels.push(...prevChannels);
    }

    // Si main a réussi, on évite de dupliquer les extras déjà dans prevChannels
    // On nettoie les anciennes extras, puis on ajoute les nouvelles (ou fallback)
    const hadFreshMain = Array.isArray(mainParsed) && mainParsed.length;
    if (hadFreshMain) {
      // Retire toutes les entrées provenant des extras (elles seront re-ajoutées)
      const extraSet = new Set(EXTRA_PLAYLIST_URLS);
      for (let i = nextChannels.length - 1; i >= 0; i--) {
        if (nextChannels[i] && extraSet.has(nextChannels[i].sourceId)) {
          nextChannels.splice(i, 1);
        }
      }

      // Ajoute chaque extra : parsed si OK, sinon fallback depuis prevChannels
      EXTRA_PLAYLIST_URLS.forEach((u, idx) => {
        const parsed = extrasResults[idx];
        if (Array.isArray(parsed) && parsed.length) {
          nextChannels.push(...parsed);
        } else {
          const fallback = prevChannels.filter(e => e && e.sourceId === u);
          if (fallback.length) nextChannels.push(...fallback);
        }
      });
    }
    ...
    // Applique FR (fallback si échec)
    if (Array.isArray(frParsed) && frParsed.length) {
      frChannels.splice(0, frChannels.length, ...frParsed);
    } else {
      frChannels.splice(0, frChannels.length, ...prevFr);
    }
  } catch (err) {
    console.warn('[Auto-refresh] échec global', err);
  } finally {
    suspendRender = false;
  }
...
(async function loadMainPlaylists() {
  ...
  suspendRender = true;
  try {
    // ✅ Chargement séquentiel pour éviter une course (race condition) :
      // si les playlists "extra" finissent avant la principale (append:false),
      // la principale peut effacer channels[] et faire disparaître des catégories.
      // On charge donc : main (reset) → extras (append) → FR.
      await loadFromUrl(MAIN_PLAYLIST_URL, { autoplay: false, append: false });
      await Promise.all(loadExtraPlaylists({ autoplay: false, append: true }));
      await loadFrM3u(FR_PLAYLIST_URL);
...
// ✅ Akamai-style: lecture directe via ?streamUrl=...
  ...
  // 🎬 Intro (loop) uniquement pour l’onglet Films (channelList)
  // Sinon, on garde le comportement normal : autoplay de la liste active.
  try {
    const activeTab = getActiveTabKey ? getActiveTabKey() : (document.querySelector('.tab-btn.active')?.dataset?.tab || currentListType || '');
    if (activeTab === 'channels') {
      playIntroLoop();
    } else {
      autoplayFirstInList(activeTab || currentListType || 'fr');
    }
  } catch (_) {}
...
const tabVitrineBtn = document.getElementById('tabVitrineBtn');
...
function __createShowcaseCard(entry, index, showNewBadge){
  ...
  // ✅ NOUVEAU: expose audio/subs au DOM (pour multi-lang-badge.js)
  if (entry?.audio) card.dataset.audio = String(entry.audio);
  if (entry?.subs)  card.dataset.subs  = String(entry.subs);
...
async function openTmdbSynopsisBackdrop(entry, index){
  ...
  if(!data.ok){
    ...
    const y = (releaseDate || '').slice(0,4) || __extractYearFromName(entry?.name);
    const runtime = __formatRuntime((typeof data.runtime === 'number') ? data.runtime : null);
    const rawRating = (typeof data.rating === 'number')
      ? data.rating
      : ((typeof data.vote_average === 'number') ? data.vote_average : null);

    const rating = (rawRating !== null) ? (Math.round(rawRating * 10) / 10) : null;
    const voteCountRaw = (typeof data.vote_count === 'number')
      ? data.vote_count
      : ((typeof data.voteCount === 'number') ? data.voteCount : null);

    const fmtScore = (v) => {
      try { return new Intl.NumberFormat('fr-FR', { minimumFractionDigits: 1, maximumFractionDigits: 1 }).format(v); }
      catch { return String(v); }
    };

    const fmtInt = (v) => {
      try { return new Intl.NumberFormat('fr-FR').format(v); }
      catch { return String(v); }
    };
    ...
    if(mEl){
      ...
      if (rating !== null) {
        const scoreTxt = fmtScore(rating);
        const votesTxt = (voteCountRaw !== null) ? fmtInt(voteCountRaw) : '';
        const votesSpan = votesTxt ? ` <span class="tmdb-rating-count">(${votesTxt} votes)</span>` : '';
        parts.push(`<span class="tmdb-rating"><span class="rating-star">★</span> ${scoreTxt}/10${votesSpan}</span>`);
      }
      ...
      try {
        const votesTxt = (voteCountRaw !== null) ? fmtInt(voteCountRaw) : '';
        const scoreTxt = (rating !== null) ? fmtScore(rating) : '';
        const aria = (rating !== null)
          ? (`Score d’évaluation ${scoreTxt} sur 10` + (votesTxt ? `, basé sur ${votesTxt} votes` : ''))
          : 'Score d’évaluation indisponible';
        scoreEl.setAttribute('aria-label', aria);
      } catch {}
      __renderUserScoreChart(scoreEl, rating);
    }
...
(function __initShowcase(){
  ...
  // Compat: certains layouts utilisent un bouton dédié "tabVitrineBtn"
  // qui doit (1) activer l’onglet Films puis (2) ouvrir la vitrine.
  if(tabVitrineBtn){
    tabVitrineBtn.addEventListener('click', () => {
      try{
        const filmsTabBtn = document.querySelector('.tab-btn[data-tab="channels"]');
        if(filmsTabBtn && !filmsTabBtn.classList.contains('active')) filmsTabBtn.click();
      }catch(_){ }
      // laisse le temps au DOM d’appliquer l’onglet actif
      requestAnimationFrame(() => {
        requestAnimationFrame(() => openShowcase());
      });
    });
  }
...
/* =========================================================
   HELP SIDEBAR (ⓘ) – rappel des fonctions
   - Ajoute une icône ⓘ après .helper-text
   - Ouvre une sidebar "help" (droite)
   - Ferme: clic backdrop / ✕ / ESC
   ========================================================= */
(function () {
  "use strict";

  const FEATURES_LIST = [
    "Player HLS / DASH / MP4 / Audio",
    "Moteur M3U",
    "Moteur de sous-titres multi-providers avec API",
    "Gestion favoris / historique / reprise",
    "Chromecast",
    "PWA offline",
    "Overlay iFrame intelligent",
    "Overlay Torrents intelligent",
    "UI onglets dynamique",
    "Moteur de recherche global",
    "Système “derniers ajouts”"
  ];
  ...
  function ensureStyles() {
    if (document.getElementById("helpSidebarStyles")) return;
    const style = ce("style");
    style.id = "helpSidebarStyles";
    style.textContent = `
      .help-i-btn{
      ...
      .help-i-btn:hover{ filter:brightness(1.1); }
      .help-i-btn:active{ transform:translateY(1px); }
      ...
      .help-sidebar-backdrop.hidden{ display:none; }
      ...
      .help-sidebar-close:hover{ filter:brightness(1.1); }

      .help-sidebar-list{
        margin:0; padding:0 0 0 18px;
        color:rgba(217,255,255,.92);
        font-size:13px;
        line-height:1.55;
      }
      ...
      .help-sidebar-foot{
        margin-top:14px;
        font-size:12px;
        color:rgba(217,255,255,.65);
      }
    `;
    document.head.appendChild(style);
  }

  function buildSidebarIfNeeded() {
    let backdrop = document.getElementById("helpSidebarBackdrop");
    if (backdrop) return backdrop;

    ensureStyles();

    backdrop = ce("div", "help-sidebar-backdrop hidden");
    backdrop.id = "helpSidebarBackdrop";
    backdrop.setAttribute("aria-hidden", "true");

    const panel = ce("aside", "help-sidebar");
    panel.setAttribute("role", "dialog");
    panel.setAttribute("aria-modal", "true");
    panel.setAttribute("aria-label", "Help");

    const header = ce("div", "help-sidebar-header");
    const title = ce("div", "help-sidebar-title");
    title.textContent = "Aide · Rappel des fonctions";

    const closeBtn = ce("button", "help-sidebar-close");
    closeBtn.type = "button";
    closeBtn.id = "helpSidebarCloseBtn";
    ...
    const ul = ce("ul", "help-sidebar-list");
    FEATURES_LIST.forEach((t) => {
      const li = ce("li");
      li.textContent = t;
      ul.appendChild(li);
    });

    const foot = ce("div", "help-sidebar-foot");
    foot.textContent = "Astuce : tu peux garder cette sidebar ouverte pendant la navigation.";
    ...
    backdrop.addEventListener("click", (ev) => {
      if (ev.target === backdrop) closeSidebar();
    });
  ...
  function openSidebar() {
    const backdrop = buildSidebarIfNeeded();
  ...
  function injectInfoIcon() {
    const helper = qs(".helper-text");
    if (!helper) return;

    // évite les doublons
    if (qs("#helpInfoBtn", helper.parentElement || document)) return;

    const btn = ce("button", "help-i-btn");
    btn.type = "button";
    btn.id = "helpInfoBtn";
    btn.title = "Aide / fonctions";
    btn.setAttribute("aria-label", "Aide / fonctions");
    btn.textContent = "ⓘ";
    ...
    // on l'ajoute juste après le helper-text
    helper.insertAdjacentElement("afterend", btn);
  }
...
// =====================================================
// DIRECT STREAM MEMORY MANAGER (localStorage + export/import JSON + history)
// =====================================================
(function __aresDirectMemoryManager(){
  const MEMORY_EXPORT_VERSION = 1;
  const HISTORY_KEY = 'tronAres.directSubtitleHistory.v1';
  const HISTORY_LIMIT = 80;
  const MEMORY_EXPORT_FILENAME = 'tron-ares-direct-memory.json';

  let __directSubtitleHistory = (() => {
    try {
  ...
  function __persistDirectSubtitleHistory(){
    try {
      __directSubtitleHistory = (Array.isArray(__directSubtitleHistory) ? __directSubtitleHistory : [])
        .filter(Boolean)
        .slice(0, HISTORY_LIMIT);
      localStorage.setItem(HISTORY_KEY, JSON.stringify(__directSubtitleHistory));
    } catch {}
  }

  function __safeNowIso(){
    try { return new Date().toISOString(); } catch { return ''; }
  }

  function __buildMemoryRecord(url, payload = {}) {
    const key = __subtitleBuildDirectUrlKey(url);
    if (!key) return null;
    const prev = (__directSubtitleMemory[key] && typeof __directSubtitleMemory[key] === 'object')
      ? __directSubtitleMemory[key]
    ...
    const title = String(payload.title || prev.title || '').trim();
    const queryLabel = String(payload.queryLabel || prev.queryLabel || title || '').trim();
    const provider = String(
      payload.provider
      || prev.provider
      || (typeof subtitleSearchProviderSelect !== 'undefined' && subtitleSearchProviderSelect ? subtitleSearchProviderSelect.value : '')
      || 'opensubtitles'
    ).trim() || 'opensubtitles';
    const langsRaw = payload.langs != null
      ? payload.langs
      : (prev.langs != null
          ? prev.langs
          : (typeof subtitleSearchLangInput !== 'undefined' && subtitleSearchLangInput ? subtitleSearchLangInput.value : ''));
    const langs = Array.isArray(langsRaw)
      ? langsRaw.map(v => String(v || '').trim()).filter(Boolean)
      : String(langsRaw || '').split(',').map(v => v.trim()).filter(Boolean);

    const tmdbId = String(payload.tmdbId || prev.tmdbId || '').trim();
    const tmdbType = String(payload.tmdbType || prev.tmdbType || '').trim() || 'movie';
    const year = String(payload.year || prev.year || '').trim();
    ...
    return {
      ...
      provider,
      langs,
      at: Number(payload.at || prev.at || timestamp) || timestamp,
      updatedAt: timestamp,
      updatedAtIso: __safeNowIso()
    };
  }

  function __upsertHistoryFromMemoryItem(item){
    if (!item || !item.key) return;
    const base = {
      key: item.key,
      url: String(item.url || '').trim(),
      title: String(item.title || '').trim(),
      queryLabel: String(item.queryLabel || item.title || '').trim(),
      year: String(item.year || '').trim(),
      tmdbId: String(item.tmdbId || '').trim(),
      tmdbType: String(item.tmdbType || 'movie').trim() || 'movie',
      provider: String(item.provider || 'opensubtitles').trim() || 'opensubtitles',
      langs: Array.isArray(item.langs) ? item.langs.slice(0, 12) : [],
      lastPlayedAt: Date.now(),
      lastPlayedAtIso: __safeNowIso()
    };
    const list = Array.isArray(__directSubtitleHistory) ? __directSubtitleHistory : [];
    const filtered = list.filter(row => row && row.key !== item.key);
    filtered.unshift(base);
    __directSubtitleHistory = filtered.slice(0, HISTORY_LIMIT);
    __persistDirectSubtitleHistory();
  }

  function __mergeImportedMemoryBlock(block){
    const input = (block && typeof block === 'object') ? block : {};
    const merged = { ...( __directSubtitleMemory && typeof __directSubtitleMemory === 'object' ? __directSubtitleMemory : {} ) };

    for (const [maybeKey, rawItem] of Object.entries(input)) {
      if (!rawItem || typeof rawItem !== 'object') continue;
      const url = String(rawItem.url || '').trim();
      const key = __subtitleBuildDirectUrlKey(url || maybeKey);
      if (!key) continue;
      const item = __buildMemoryRecord(url || rawItem.url || '', rawItem);
      if (!item) continue;
      const prev = merged[key];
      const prevAt = Number(prev?.updatedAt || prev?.at || 0);
      const nextAt = Number(item.updatedAt || item.at || 0);
      if (!prev || nextAt >= prevAt) merged[key] = item;
    }

    __directSubtitleMemory = merged;
    try { __persistDirectSubtitleMemory(); } catch {}
  }

  function __mergeImportedHistoryBlock(rows){
    if (!Array.isArray(rows)) return;
    const seen = new Set();
    const merged = [];
    const source = rows.concat(Array.isArray(__directSubtitleHistory) ? __directSubtitleHistory : []);
    source.forEach((row) => {
      if (!row || typeof row !== 'object') return;
      const key = __subtitleBuildDirectUrlKey(row.url || row.key || '');
      ...
      merged.push({
        ...
        queryLabel: String(row.queryLabel || row.title || '').trim(),
        year: String(row.year || '').trim(),
        tmdbId: String(row.tmdbId || '').trim(),
        tmdbType: String(row.tmdbType || 'movie').trim() || 'movie',
        provider: String(row.provider || 'opensubtitles').trim() || 'opensubtitles',
        langs: Array.isArray(row.langs) ? row.langs.map(v => String(v || '').trim()).filter(Boolean).slice(0,12) : [],
        lastPlayedAt: Number(row.lastPlayedAt || Date.now()) || Date.now(),
        lastPlayedAtIso: String(row.lastPlayedAtIso || __safeNowIso()).trim()
      });
    });
    __directSubtitleHistory = merged
      .sort((a,b) => Number(b.lastPlayedAt || 0) - Number(a.lastPlayedAt || 0))
      .slice(0, HISTORY_LIMIT);
    __persistDirectSubtitleHistory();
  }

  function __buildExportPayload(){
    const memoryEntries = Object.entries(__directSubtitleMemory || {})
      .sort((a, b) => Number(b?.[1]?.updatedAt || b?.[1]?.at || 0) - Number(a?.[1]?.updatedAt || a?.[1]?.at || 0));
    return {
      app: 'Ares Media Engine',
      kind: 'direct-stream-memory',
      version: MEMORY_EXPORT_VERSION,
      exportedAt: Date.now(),
      exportedAtIso: __safeNowIso(),
      memoryKey: __DIRECT_SUBTITLE_MEMORY_KEY,
      historyKey: HISTORY_KEY,
      streams: Object.fromEntries(memoryEntries),
      history: Array.isArray(__directSubtitleHistory) ? __directSubtitleHistory.slice(0, HISTORY_LIMIT) : []
    };
  }

  function __downloadTextFile(filename, content, mime = 'application/json;charset=utf-8'){
    try {
  ...
  async function __readFileAsText(file){
  ...
  function __formatRelativeDate(timestamp){
    ...
    if (diff < day) return 'il y a ' + Math.max(1, Math.round(diff / hour)) + ' h';
    return 'il y a ' + Math.max(1, Math.round(diff / day)) + ' j';
  }

  function __openRememberedStreamFromHistory(item){
    if (!item || !item.url) return;
    try {
      if (typeof openStreamUrlPanel === 'function') openStreamUrlPanel(false);
    } catch {}
    try { if (typeof streamUrlInput !== 'undefined' && streamUrlInput) streamUrlInput.value = item.url || ''; } catch {}
    try { if (typeof streamTitleInput !== 'undefined' && streamTitleInput) streamTitleInput.value = item.queryLabel || item.title || ''; } catch {}
    try {
      if (typeof playDirectStream === 'function') {
        playDirectStream(item.url, item.queryLabel || item.title || '', { updateUrl: true });
      }
    } catch (e) {
      console.error(e);
    }
  ...
  function __removeRememberedMemoryItem(item){
    if (!item || !item.url) return;
    try { __subtitleForgetForUrl(item.url); } catch {}
    __directSubtitleHistory = (Array.isArray(__directSubtitleHistory) ? __directSubtitleHistory : [])
      .filter(row => row && row.key !== __subtitleBuildDirectUrlKey(item.url));
    __persistDirectSubtitleHistory();
    try { __renderDirectMemoryHistoryUi(); } catch {}
    try { setStatus('Mémoire supprimée pour ce flux'); } catch {}
  }

  function __clearAllDirectMemory(){
    __directSubtitleMemory = {};
    __directSubtitleHistory = [];
    try { localStorage.removeItem(__DIRECT_SUBTITLE_MEMORY_KEY); } catch {}
    try { localStorage.removeItem(HISTORY_KEY); } catch {}
    try { __renderDirectMemoryHistoryUi(); } catch {}
    try { setStatus('Mémoire directe vidée'); } catch {}
  }

  function __importDirectMemoryFromText(rawText){
    const text = String(rawText || '').trim();
    if (!text) throw new Error('Fichier vide');
    const parsed = JSON.parse(text);
    const streams = parsed?.streams && typeof parsed.streams === 'object'
      ? parsed.streams
      : (parsed?.memory && typeof parsed.memory === 'object' ? parsed.memory : null);
    if (!streams || typeof streams !== 'object') throw new Error('Format mémoire invalide');
    __mergeImportedMemoryBlock(streams);
    if (Array.isArray(parsed?.history)) __mergeImportedHistoryBlock(parsed.history);
    try { __renderDirectMemoryHistoryUi(); } catch {}
    const count = Object.keys(__directSubtitleMemory || {}).length;
    try { setStatus('Mémoire importée (' + count + ' flux)'); } catch {}
    return count;
  }

  function __exportDirectMemoryJson(){
    const payload = __buildExportPayload();
    const json = JSON.stringify(payload, null, 2);
    const ok = __downloadTextFile(MEMORY_EXPORT_FILENAME, json);
    if (ok) {
      try { setStatus('Export mémoire téléchargé'); } catch {}
    } else {
      try { window.prompt('Copie le JSON mémoire :', json); } catch {}
    }
  }

  function __ensureDirectMemoryUi(){
    const overlay = document.getElementById('subtitleSearchOverlay');
    const results = document.getElementById('subtitleSearchResults');
    if (!overlay || !results) return null;

    let wrap = document.getElementById('directMemoryUiWrap');
    let fileInput = document.getElementById('directMemoryImportInput');
    let exportBtn = document.getElementById('directMemoryExportBtn');
    let importBtn = document.getElementById('directMemoryImportBtn');
    let clearBtn = document.getElementById('directMemoryClearBtn');
    let historyBtn = document.getElementById('directMemoryHistoryBtn');
    let panel = document.getElementById('directMemoryHistoryPanel');
    let stats = document.getElementById('directMemoryLibraryStats');
    let searchInput = document.getElementById('directMemoryLibrarySearch');
    let sortSelect = document.getElementById('directMemoryLibrarySort');

    if (!wrap) {
      wrap = document.createElement('div');
      wrap.id = 'directMemoryUiWrap';
      ...
      exportBtn.id = 'directMemoryExportBtn';
      exportBtn.className = 'btn btn-ghost';
      exportBtn.textContent = 'Exporter mémoire';

      importBtn = document.createElement('button');
      importBtn.type = 'button';
      importBtn.id = 'directMemoryImportBtn';
      importBtn.className = 'btn btn-ghost';
      importBtn.textContent = 'Importer mémoire';

      clearBtn = document.createElement('button');
      clearBtn.type = 'button';
      clearBtn.id = 'directMemoryClearBtn';
      clearBtn.className = 'btn btn-ghost';
      clearBtn.textContent = 'Vider mémoire';

      historyBtn = document.createElement('button');
      historyBtn.type = 'button';
      historyBtn.id = 'directMemoryHistoryBtn';
      historyBtn.className = 'btn btn-accent';
      historyBtn.textContent = 'Bibliothèque flux';

      actions.append(exportBtn, importBtn, clearBtn, historyBtn);

      const hint = document.createElement('div');
      hint.className = 'subsearch-hint';
      hint.style.marginTop = '6px';
      hint.textContent = 'La mémoire directe est stockée en localStorage, exportable en JSON et réimportable après nettoyage navigateur.';

      panel = document.createElement('div');
      panel.id = 'directMemoryHistoryPanel';
      panel.className = 'subsearch-results';
      panel.style.marginTop = '8px';
      panel.style.display = 'none';
      panel.style.padding = '10px';
      panel.style.borderRadius = '14px';
      panel.style.border = '1px solid rgba(255,145,0,.16)';
      panel.style.background = 'rgba(7,10,17,.45)';

      const toolbar = document.createElement('div');
      toolbar.id = 'directMemoryLibraryToolbar';
      toolbar.style.display = 'grid';
      toolbar.style.gridTemplateColumns = '1fr minmax(170px, 220px)';
      toolbar.style.gap = '8px';
      toolbar.style.marginBottom = '10px';

      searchInput = document.createElement('input');
      searchInput.type = 'text';
      searchInput.id = 'directMemoryLibrarySearch';
      searchInput.className = 'subsearch-input';
      searchInput.placeholder = 'Rechercher dans la bibliothèque…';
      searchInput.autocomplete = 'off';
      searchInput.spellcheck = false;

      sortSelect = document.createElement('select');
      sortSelect.id = 'directMemoryLibrarySort';
      sortSelect.className = 'subsearch-select';
      [
        ['recent','Plus récents'],
        ['title','Titre A → Z'],
        ['tmdb','TMDb d’abord']
      ...
      stats.id = 'directMemoryLibraryStats';
      stats.className = 'subsearch-hint';
      stats.style.margin = '2px 0 10px';

      const list = document.createElement('div');
      list.id = 'directMemoryLibraryList';
      list.style.display = 'grid';
      list.style.gridTemplateColumns = 'repeat(auto-fit, minmax(240px, 1fr))';
      list.style.gap = '10px';

      toolbar.append(searchInput, sortSelect);
      panel.append(toolbar, stats, list);

      fileInput = document.createElement('input');
      fileInput.type = 'file';
      fileInput.accept = '.json,application/json';
      fileInput.id = 'directMemoryImportInput';
      fileInput.style.display = 'none';

      wrap.append(actions, hint, panel, fileInput);
      results.parentNode.insertBefore(wrap, results.nextSibling);
    }

    if (exportBtn && !exportBtn.dataset.bound) {
      exportBtn.dataset.bound = '1';
      exportBtn.addEventListener('click', () => { __exportDirectMemoryJson(); });
    }
    ...
    if (fileInput && !fileInput.dataset.bound) {
      ...
      fileInput.addEventListener('change', async () => {
        ...
        if (!file) return;
        try {
          const text = await __readFileAsText(file);
          __importDirectMemoryFromText(text);
        } catch (e) {
          console.error(e);
          try { setStatus('Import impossible : ' + (e && e.message ? e.message : String(e))); } catch {}
        } finally {
          try { fileInput.value = ''; } catch {}
        }
      });
    ...
    if (clearBtn && !clearBtn.dataset.bound) {
      ...
      clearBtn.addEventListener('click', () => {
        const ok = window.confirm('Vider toute la mémoire des flux directs et l’historique ?');
        if (!ok) return;
        __clearAllDirectMemory();
      });
    }
    if (historyBtn && !historyBtn.dataset.bound) {
      ...
      historyBtn.addEventListener('click', () => {
        ...
        historyBtn.textContent = hidden ? 'Masquer bibliothèque' : 'Bibliothèque flux';
        if (hidden) __renderDirectMemoryHistoryUi();
      });
    }
    if (searchInput && !searchInput.dataset.bound) {
      searchInput.dataset.bound = '1';
      searchInput.addEventListener('input', () => { __renderDirectMemoryHistoryUi(); });
    }
    if (sortSelect && !sortSelect.dataset.bound) {
      sortSelect.dataset.bound = '1';
      sortSelect.addEventListener('change', () => { __renderDirectMemoryHistoryUi(); });
    }

    return { wrap, fileInput, exportBtn, importBtn, clearBtn, historyBtn, panel, stats, searchInput, sortSelect };
  }

  function __renderDirectMemoryHistoryUi(){
    const ui = __ensureDirectMemoryUi();
    if (!ui || !ui.panel) return;
    const panel = ui.panel;
    const list = document.getElementById('directMemoryLibraryList');
    const stats = ui.stats || document.getElementById('directMemoryLibraryStats');
    if (!list) return;
    list.innerHTML = '';

    const memoryItems = Object.values(__directSubtitleMemory || {}).filter(Boolean);
    const historyItems = (Array.isArray(__directSubtitleHistory) ? __directSubtitleHistory : []).filter(Boolean);
    const map = new Map();

    memoryItems.forEach((item) => {
      if (item && item.url) map.set(item.key || __subtitleBuildDirectUrlKey(item.url), Object.assign({}, item));
    });
    historyItems.forEach((item) => {
      if (!item || !item.url) return;
      const key = item.key || __subtitleBuildDirectUrlKey(item.url);
      const previous = map.get(key) || {};
      map.set(key, Object.assign({}, previous, item, {
        title: previous.title || item.title,
        queryLabel: previous.queryLabel || item.queryLabel || previous.title || item.title,
        updatedAt: Math.max(Number(previous.updatedAt || previous.at || 0), Number(item.updatedAt || item.at || 0)) || previous.updatedAt || item.updatedAt,
        lastPlayedAt: Math.max(Number(previous.lastPlayedAt || 0), Number(item.lastPlayedAt || 0)) || previous.lastPlayedAt || item.lastPlayedAt,
      }));
    });

    let items = Array.from(map.values()).filter(Boolean);
    const query = String((ui.searchInput && ui.searchInput.value) || '').trim().toLowerCase();
    if (query) {
      items = items.filter((item) => {
        const hay = [item.queryLabel, item.title, item.url, item.provider, item.tmdbId]
          .filter(Boolean)
          .join(' ')
          .toLowerCase();
    ...
    const sortMode = String((ui.sortSelect && ui.sortSelect.value) || 'recent');
    if (sortMode === 'title') {
      items.sort((a,b) => String(a.queryLabel || a.title || a.url || '').localeCompare(String(b.queryLabel || b.title || b.url || ''), 'fr', { sensitivity: 'base' }));
    } else if (sortMode === 'tmdb') {
      items.sort((a,b) => {
        const aScore = a.tmdbId ? 1 : 0;
        const bScore = b.tmdbId ? 1 : 0;
        if (bScore !== aScore) return bScore - aScore;
        return Number(b.lastPlayedAt || b.updatedAt || b.at || 0) - Number(a.lastPlayedAt || a.updatedAt || a.at || 0);
      });
    } else {
      items.sort((a,b) => Number(b.lastPlayedAt || b.updatedAt || b.at || 0) - Number(a.lastPlayedAt || a.updatedAt || a.at || 0));
    }

    if (stats) {
      const total = map.size;
      const shown = items.length;
      stats.textContent = shown + ' flux affiché' + (shown > 1 ? 's' : '') + ' sur ' + total + ' mémorisé' + (total > 1 ? 's' : '') + '.';
    }

    if (!items.length) {
      const empty = document.createElement('div');
      empty.className = 'subsearch-result-meta';
      empty.textContent = query ? 'Aucun flux trouvé pour cette recherche.' : 'Aucun flux direct mémorisé pour le moment.';
      list.appendChild(empty);
      return;
    }

    const frag = document.createDocumentFragment();
    items.slice(0, HISTORY_LIMIT * 2).forEach((item) => {
      const card = document.createElement('div');
      card.className = 'subsearch-result-card';
      card.style.display = 'flex';
      card.style.flexDirection = 'column';
      ...
      const title = document.createElement('div');
      title.className = 'subsearch-result-title';
      title.textContent = item.queryLabel || item.title || item.url || 'Flux direct';

      const meta = document.createElement('div');
      meta.className = 'subsearch-result-meta';
      const metaParts = [];
      if (item.tmdbId) metaParts.push('TMDb #' + item.tmdbId);
      if (Array.isArray(item.langs) && item.langs.length) metaParts.push(item.langs.join(', '));
      if (item.provider) metaParts.push(item.provider);
      metaParts.push('Lu ' + __formatRelativeDate(item.lastPlayedAt || item.updatedAt || item.at));
      meta.textContent = metaParts.join(' • ');

      const urlLine = document.createElement('div');
      urlLine.className = 'subsearch-result-meta';
      urlLine.style.wordBreak = 'break-all';
      urlLine.style.display = '-webkit-box';
      urlLine.style.webkitLineClamp = '2';
      urlLine.style.webkitBoxOrient = 'vertical';
      urlLine.style.overflow = 'hidden';
      urlLine.textContent = String(item.url || '');
      ...
      reuseBtn.className = 'subsearch-mini-btn';
      reuseBtn.textContent = 'Relancer';
      reuseBtn.addEventListener('click', () => { __openRememberedStreamFromHistory(item); });

      const fillBtn = document.createElement('button');
      fillBtn.type = 'button';
      fillBtn.className = 'subsearch-mini-btn';
      fillBtn.textContent = 'Préremplir';
      fillBtn.addEventListener('click', () => {
        try { if (typeof openStreamUrlPanel === 'function') openStreamUrlPanel(false); } catch {}
        try { if (typeof streamUrlInput !== 'undefined' && streamUrlInput) streamUrlInput.value = item.url || ''; } catch {}
        try { if (typeof streamTitleInput !== 'undefined' && streamTitleInput) streamTitleInput.value = item.queryLabel || item.title || ''; } catch {}
        try { setStatus('Flux mémorisé rechargé dans Stream URL'); } catch {}
      });

      const subtitleBtn = document.createElement('button');
      subtitleBtn.type = 'button';
      subtitleBtn.className = 'subsearch-mini-btn';
      subtitleBtn.textContent = 'Sous-titres';
      subtitleBtn.addEventListener('click', () => {
        try { __subtitleOpenOverlay(item.queryLabel || item.title || ''); } catch {}
        try { if (typeof subtitleSearchTitleInput !== 'undefined' && subtitleSearchTitleInput) subtitleSearchTitleInput.value = item.queryLabel || item.title || ''; } catch {}
        try { if (typeof __subtitleDoSearch === 'function') __subtitleDoSearch(); } catch {}
      });

      const deleteBtn = document.createElement('button');
      deleteBtn.type = 'button';
      deleteBtn.className = 'subsearch-mini-btn';
      deleteBtn.textContent = 'Supprimer';
      deleteBtn.addEventListener('click', () => {
        const ok = window.confirm('Supprimer ce flux de la mémoire ?');
        if (!ok) return;
        __removeRememberedMemoryItem(item);
      });

      actions.append(reuseBtn, fillBtn, subtitleBtn, deleteBtn);
      card.append(title, meta, urlLine, actions);
      frag.appendChild(card);
    });
    list.appendChild(frag);
  }

  const __origRememberForUrl = __subtitleRememberForUrl;
  __subtitleRememberForUrl = function(url, payload = {}){
    const result = __origRememberForUrl.call(this, url, payload);
    try {
      const item = __buildMemoryRecord(url, payload);
      if (item) {
        __directSubtitleMemory[item.key] = item;
        __persistDirectSubtitleMemory();
        __upsertHistoryFromMemoryItem(item);
      }
    } catch (e) {
      console.error(e);
    }
  ...
  const __origForgetForUrl = __subtitleForgetForUrl;
  __subtitleForgetForUrl = function(url){
    const result = __origForgetForUrl.call(this, url);
    try {
      const key = __subtitleBuildDirectUrlKey(url);
      __directSubtitleHistory = (Array.isArray(__directSubtitleHistory) ? __directSubtitleHistory : [])
        .filter(row => row && row.key !== key);
      __persistDirectSubtitleHistory();
    } catch (e) {
      console.error(e);
    }
  ...
  const __origOpenSubtitleOverlay = __subtitleOpenOverlay;
  __subtitleOpenOverlay = function(prefillTitle = ''){
    const result = __origOpenSubtitleOverlay.call(this, prefillTitle);
    try { __ensureDirectMemoryUi(); } catch {}
    return result;
  };

  const __origOpenStreamUrlPanel = openStreamUrlPanel;
  openStreamUrlPanel = function(prefillFromQuery = true){
    const result = __origOpenStreamUrlPanel.call(this, prefillFromQuery);
    try { __ensureDirectMemoryUi(); } catch {}
    return result;
  };

  window.__aresExportDirectMemoryJson = __exportDirectMemoryJson;
  window.__aresImportDirectMemoryFromText = __importDirectMemoryFromText;
  window.__aresRenderDirectMemoryHistoryUi = __renderDirectMemoryHistoryUi;

  try { __ensureDirectMemoryUi(); } catch {}
})();


/* ===== Ares direct memory library posters (TMDb) ===== */
(function(){
  const MEMORY_KEY = 'tronAres.directSubtitleMemory.v1';
  const HISTORY_KEY = 'tronAres.directSubtitleHistory.v1';
  const IMG_FALLBACK = 'data:image/svg+xml;utf8,' + encodeURIComponent(
    '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 240 360">'
    + '<defs><linearGradient id="g" x1="0" y1="0" x2="1" y2="1"><stop offset="0" stop-color="#00e5ff"/><stop offset="1" stop-color="#ff9100"/></linearGradient></defs>'
    + '<rect width="240" height="360" rx="22" fill="#0b1018"/>'
    + '<rect x="18" y="18" width="204" height="324" rx="18" fill="none" stroke="url(#g)" stroke-width="4" opacity=".7"/>'
    + '<text x="50%" y="48%" fill="#dbe7ff" font-size="34" text-anchor="middle" font-family="Arial, sans-serif">Ares</text>'
    + '<text x="50%" y="58%" fill="#90a4c7" font-size="18" text-anchor="middle" font-family="Arial, sans-serif">Flux mémorisé</text>'
    + '</svg>'
  );
  ...
  function getLibraryMap(){
    ...
    const map = new Map();
    Object.values(memory).filter(Boolean).forEach((item) => {
      if (!item || !item.url) return;
      const key = item.key || directUrlKey(item.url);
    ...
    history.filter(Boolean).forEach((item) => {
      ...
      map.set(key, Object.assign({}, prev, item, {
        ...
        queryLabel: prev.queryLabel || item.queryLabel || prev.title || item.title,
        updatedAt: Math.max(Number(prev.updatedAt || prev.at || 0), Number(item.updatedAt || item.at || 0)) || prev.updatedAt || item.updatedAt,
        lastPlayedAt: Math.max(Number(prev.lastPlayedAt || 0), Number(item.lastPlayedAt || 0)) || prev.lastPlayedAt || item.lastPlayedAt,
      }));
    });
    return map;
  }

  function findItemForCard(card, libMap){
    const metaNodes = Array.from(card.querySelectorAll('.subsearch-result-meta'));
    const urlLine = metaNodes.find((el) => /^https?:/i.test(String(el.textContent || '').trim()));
    const url = String(urlLine && urlLine.textContent || '').trim();
    if (!url) return null;
    return libMap.get(directUrlKey(url)) || null;
  }

  function ensurePosterLayout(card){
    if (card.dataset.posterLayoutReady === '1') return;
    const title = card.querySelector('.subsearch-result-title');
    const metas = Array.from(card.querySelectorAll('.subsearch-result-meta'));
    const actions = Array.from(card.children).find((el) => el.tagName === 'DIV' && Array.from(el.querySelectorAll('button')).length >= 2);
    if (!title || !actions) return;

    const posterWrap = document.createElement('div');
    posterWrap.className = 'ares-library-poster-wrap';
    posterWrap.style.width = '86px';
    posterWrap.style.minWidth = '86px';
    posterWrap.style.alignSelf = 'stretch';

    const poster = document.createElement('img');
    poster.className = 'ares-library-poster';
    poster.alt = 'Poster';
    poster.loading = 'lazy';
    poster.decoding = 'async';
    poster.referrerPolicy = 'no-referrer';
    poster.src = IMG_FALLBACK;
    poster.style.width = '86px';
    poster.style.height = '129px';
    poster.style.objectFit = 'cover';
    poster.style.borderRadius = '14px';
    poster.style.border = '1px solid rgba(255,145,0,.22)';
    poster.style.background = 'rgba(10,14,22,.85)';
    poster.style.boxShadow = '0 8px 24px rgba(0,0,0,.35)';
    posterWrap.appendChild(poster);

    const body = document.createElement('div');
    body.className = 'ares-library-body';
    ...
    card.insertBefore(posterWrap, card.firstChild);
    body.appendChild(title);
    metas.forEach((el) => body.appendChild(el));
    body.appendChild(actions);
    card.appendChild(body);
    card.dataset.posterLayoutReady = '1';
  }

  async function resolvePoster(item){
    if (!item) return '';
    const key = String(item.key || item.url || item.tmdbId || item.queryLabel || item.title || '').trim();
    if (posterCache.has(key)) return posterCache.get(key) || '';

    let url = '';
    try {
      if (item.posterUrl && /^https?:/i.test(String(item.posterUrl))) {
        url = String(item.posterUrl);
      } else if (typeof __resolveTmdbPosterUrl === 'function') {
        url = await __resolveTmdbPosterUrl(item);
      }
    } catch (e) {
      console.warn('[Ares posters] resolve failed', e);
    }
    ...
    if (url) {
      try {
        const mem = readJson(MEMORY_KEY) || {};
        const memKey = item.key || directUrlKey(item.url);
        if (mem[memKey] && mem[memKey].posterUrl !== url) {
          mem[memKey].posterUrl = url;
  ...
  async function decorateCard(card, libMap){
    if (!card || card.dataset.posterDecorating === '1') return;
    card.dataset.posterDecorating = '1';
    ensurePosterLayout(card);
    const poster = card.querySelector('.ares-library-poster');
    if (!poster) return;
    const item = findItemForCard(card, libMap);
    if (!item) {
      card.dataset.posterDecorating = '0';
      return;
    }
    ...
    if (item.tmdbId) {
      ...
      badge.style.background = 'rgba(7,10,17,.72)';
      badge.style.border = '1px solid rgba(0,229,255,.22)';
      badge.style.color = '#d7faff';
      const wrap = card.querySelector('.ares-library-poster-wrap');
      if (wrap && !wrap.querySelector('.ares-library-tmdb-badge')) {
        wrap.style.position = 'relative';
        badge.className = 'ares-library-tmdb-badge';
        wrap.appendChild(badge);
      }
    }

    if (card.dataset.posterResolved === '1') {
      card.dataset.posterDecorating = '0';
      return;
    }
    const url = await resolvePoster(item);
    if (url) poster.src = url;
    card.dataset.posterResolved = '1';
    card.dataset.posterDecorating = '0';
  }
  ...
  function decorateLibrary(){
    const list = document.getElementById('directMemoryLibraryList');
    if (!list) return;
    const libMap = getLibraryMap();
    const cards = Array.from(list.querySelectorAll('.subsearch-result-card'));
    cards.forEach((card) => { decorateCard(card, libMap); });
  }
  ...
  function installObserver(){
    ...
    observer = new MutationObserver(() => { decorateLibrary(); });
    observer.observe(list, { childList: true, subtree: true });
    decorateLibrary();
  }

  const origRender = window.__aresRenderDirectMemoryHistoryUi;
  if (typeof origRender === 'function') {
    window.__aresRenderDirectMemoryHistoryUi = function(){
      const result = origRender.apply(this, arguments);
      setTimeout(() => {
        try { installObserver(); } catch {}
        try { decorateLibrary(); } catch {}
      }, 0);
      return result;
    };
  }
  ...
  document.addEventListener('click', (ev) => {
    ...
    if (t.closest('#directMemoryHistoryBtn') || t.closest('#directMemoryLibrarySort') || t.closest('#directMemoryLibrarySearch')) {
      setTimeout(() => {
        try { installObserver(); } catch {}
        try { decorateLibrary(); } catch {}
      }, 0);
  ...
  document.addEventListener('input', (ev) => {
    const t = ev.target;
    if (t && t.id === 'directMemoryLibrarySearch') {
      setTimeout(() => { try { decorateLibrary(); } catch {} }, 0);
    }
  }, true);
...
/* ===== Ares direct memory library visual detail sheet ===== */
(function(){
  const MEMORY_KEY = 'tronAres.directSubtitleMemory.v1';
  const HISTORY_KEY = 'tronAres.directSubtitleHistory.v1';
  const TMDB_LANG_PRIMARY_SAFE = (typeof TMDB_LANG_PRIMARY !== 'undefined' && TMDB_LANG_PRIMARY) ? TMDB_LANG_PRIMARY : 'fr-FR';
  const TMDB_LANG_FALLBACK_SAFE = (typeof TMDB_LANG_FALLBACK !== 'undefined' && TMDB_LANG_FALLBACK) ? TMDB_LANG_FALLBACK : 'en-US';
  ...
  const FALLBACK_POSTER = 'data:image/svg+xml;utf8,' + encodeURIComponent(
    ...
    + '<defs><linearGradient id="g" x1="0" y1="0" x2="1" y2="1"><stop offset="0" stop-color="#00e5ff"/><stop offset="1" stop-color="#ff9100"/></linearGradient></defs>'
    + '<rect width="300" height="450" rx="26" fill="#0b1018"/>'
    + '<rect x="22" y="22" width="256" height="406" rx="22" fill="none" stroke="url(#g)" stroke-width="4" opacity=".75"/>'
    + '<text x="50%" y="47%" fill="#dbe7ff" font-size="38" text-anchor="middle" font-family="Arial, sans-serif">Ares</text>'
    + '<text x="50%" y="57%" fill="#90a4c7" font-size="18" text-anchor="middle" font-family="Arial, sans-serif">Fiche flux</text>'
    + '</svg>'
  );
  ...
  function parseUrlFromCard(card){
    const metaNodes = Array.from(card.querySelectorAll('.subsearch-result-meta'));
    const urlNode = metaNodes.find((el) => /^https?:/i.test(String(el.textContent || '').trim()));
    return String(urlNode && urlNode.textContent || '').trim();
  }

  function getCardItem(card){
    const url = parseUrlFromCard(card);
    if (!url) return null;
    return getLibraryMap().get(directUrlKey(url)) || { url };
  }
  ...
  function formatDate(ts){
    ...
    if (!n) return '';
    try {
      return new Date(n).toLocaleString('fr-FR', { dateStyle: 'medium', timeStyle: 'short' });
    } catch {
      try { return new Date(n).toLocaleString('fr-FR'); } catch { return ''; }
    }
  }

  function normalizeGenres(genres){
    if (!Array.isArray(genres)) return '';
    return genres.map((g) => g && g.name).filter(Boolean).slice(0, 4).join(' • ');
  }

  async function tmdbFetchDetails(id, lang){
    if (!id || typeof TMDB_API_KEY === 'undefined' || !TMDB_API_KEY) return null;
    const base = 'https://api.themoviedb.org/3/movie/' + encodeURIComponent(String(id));
    const p = new URLSearchParams();
    p.set('api_key', TMDB_API_KEY);
    if (lang) p.set('language', lang);
  ...
  async function getTmdbDetailsForItem(item){
    const tmdbId = String(item && item.tmdbId || '').trim();
    if (!tmdbId) return null;
    const cacheKey = tmdbId;
    if (detailsCache.has(cacheKey)) return detailsCache.get(cacheKey);
    const promise = (async () => {
      let data = null;
      try { data = await tmdbFetchDetails(tmdbId, TMDB_LANG_PRIMARY_SAFE); } catch {}
      if ((!data || !data.title) && TMDB_LANG_FALLBACK_SAFE && TMDB_LANG_FALLBACK_SAFE !== TMDB_LANG_PRIMARY_SAFE) {
        try {
          const fallback = await tmdbFetchDetails(tmdbId, TMDB_LANG_FALLBACK_SAFE);
          if (fallback) data = Object.assign({}, fallback, data || {});
        } catch {}
      }
      return data;
    })();
    ...
    try {
      ...
      detailsCache.set(cacheKey, resolved);
      return resolved;
    } catch (e) {
  ...
  function injectStyles(){
    ...
    css.id = 'aresLibraryDetailsStyles';
    css.textContent = `
      .ares-library-fiche-btn { min-width: 82px; }
      .ares-library-modal-overlay{
        position:fixed; inset:0; z-index:99999; display:none; align-items:center; justify-content:center;
        background:rgba(2,6,12,.72); backdrop-filter:blur(8px); padding:18px;
      }
      .ares-library-modal-overlay.open{ display:flex; }
      .ares-library-modal{
        width:min(1080px, 96vw); max-height:min(90vh, 920px); overflow:auto; position:relative;
        border-radius:24px; border:1px solid rgba(255,145,0,.22);
        background:linear-gradient(180deg, rgba(7,12,20,.98), rgba(10,16,27,.96));
        box-shadow:0 20px 60px rgba(0,0,0,.45);
      }
      .ares-library-modal-backdrop{
        position:absolute; inset:0; border-radius:24px; overflow:hidden; pointer-events:none;
      }
      .ares-library-modal-backdrop::after{
        content:''; position:absolute; inset:0;
        background:linear-gradient(90deg, rgba(6,11,18,.92) 0%, rgba(6,11,18,.78) 42%, rgba(6,11,18,.94) 100%);
      }
      .ares-library-modal-backdrop img{
        width:100%; height:100%; object-fit:cover; filter:blur(2px) saturate(.95); opacity:.24;
      }
      .ares-library-modal-inner{ position:relative; z-index:1; padding:22px; }
      .ares-library-modal-close{
        position:absolute; top:14px; right:14px; z-index:2; width:40px; height:40px; border-radius:999px;
        border:1px solid rgba(255,145,0,.26); background:rgba(5,10,18,.78); color:#fff; cursor:pointer;
      }
      .ares-library-modal-grid{
        display:grid; grid-template-columns:minmax(220px, 280px) 1fr; gap:20px; align-items:start;
      }
      .ares-library-modal-posterwrap{
        position:relative; border-radius:22px; overflow:hidden; border:1px solid rgba(255,145,0,.2);
        background:rgba(255,255,255,.02); aspect-ratio:2/3;
      }
      .ares-library-modal-posterwrap img{ width:100%; height:100%; object-fit:cover; display:block; }
      .ares-library-modal-posterbadge{
        position:absolute; left:10px; top:10px; padding:6px 10px; border-radius:999px; font-size:12px; font-weight:700;
        background:rgba(0,0,0,.52); color:#fff; border:1px solid rgba(255,145,0,.28);
      }
      .ares-library-modal-title{ font-size:clamp(26px,3vw,38px); font-weight:800; line-height:1.08; margin:0 0 8px; color:#fff; }
      .ares-library-modal-subtitle{ color:#c0cee7; font-size:14px; margin:0 0 14px; }
      .ares-library-modal-badges{ display:flex; gap:8px; flex-wrap:wrap; margin:0 0 14px; }
      .ares-library-modal-badge{
        padding:7px 11px; border-radius:999px; background:rgba(255,255,255,.06); color:#eef5ff; font-size:12px;
        border:1px solid rgba(255,145,0,.14);
      }
      .ares-library-modal-overview{
        color:#eaf1ff; line-height:1.58; font-size:15px; white-space:pre-wrap; margin:0 0 14px;
      }
      .ares-library-modal-meta{
        display:grid; grid-template-columns:repeat(2, minmax(0,1fr)); gap:10px; margin:0 0 16px;
      }
      .ares-library-modal-meta-card{
        padding:12px 14px; border-radius:16px; background:rgba(255,255,255,.04); border:1px solid rgba(255,145,0,.12);
      }
      .ares-library-modal-meta-label{ font-size:12px; color:#92a7c8; margin-bottom:4px; }
      .ares-library-modal-meta-value{ color:#fff; font-size:14px; word-break:break-word; }
      .ares-library-modal-actions{ display:flex; gap:10px; flex-wrap:wrap; margin-top:8px; }
      .ares-library-modal-actions button{
        min-height:40px; padding:0 14px; border-radius:12px; border:1px solid rgba(255,145,0,.18);
        background:rgba(255,255,255,.05); color:#fff; cursor:pointer;
      }
      .ares-library-modal-actions button.primary{ background:rgba(255,145,0,.18); }
      .ares-library-modal-url{
        color:#9db0cd; font-size:12px; line-height:1.5; word-break:break-all;
        padding:10px 12px; border-radius:14px; background:rgba(255,255,255,.03); border:1px solid rgba(255,145,0,.1);
      }
      .ares-library-modal-loading{ color:#c4d2ea; }
      @media (max-width: 820px){
        .ares-library-modal-grid{ grid-template-columns:1fr; }
        .ares-library-modal-posterwrap{ max-width:260px; }
        .ares-library-modal-meta{ grid-template-columns:1fr; }
      }
    `;
  ...
  function ensureModal(){
    ...
    modalRoot.id = 'aresLibraryDetailModal';
    modalRoot.className = 'ares-library-modal-overlay';
    modalRoot.innerHTML = `
      <div class="ares-library-modal" role="dialog" aria-modal="true" aria-label="Fiche du flux">
        <button type="button" class="ares-library-modal-close" title="Fermer">✕</button>
        <div class="ares-library-modal-backdrop"><img alt="Backdrop"></div>
        <div class="ares-library-modal-inner"><div class="ares-library-modal-loading">Chargement…</div></div>
      </div>
    `;
    ...
    const closeBtn = modalRoot.querySelector('.ares-library-modal-close');
  ...
  function clickCardAction(card, label){
    const btns = Array.from(card.querySelectorAll('button'));
    const btn = btns.find((b) => String(b.textContent || '').trim() === label);
    if (btn) btn.click();
  }

  function renderDetailHtml(item, details, card){
    const poster = details && details.poster_path ? (IMG_W342 + details.poster_path) : ((card.querySelector('.ares-library-poster') && card.querySelector('.ares-library-poster').src) || FALLBACK_POSTER);
    const backdrop = details && details.backdrop_path ? (IMG_ORIGINAL + details.backdrop_path) : poster;
    const title = (details && (details.title || details.original_title)) || item.queryLabel || item.title || 'Flux mémorisé';
    const releaseDate = String(details && details.release_date || '').trim();
    const year = releaseDate ? releaseDate.slice(0,4) : (item.year || '');
    const vote = details && typeof details.vote_average === 'number' && details.vote_average > 0 ? (Math.round(details.vote_average * 10) / 10).toFixed(1) : '';
    const runtime = details && details.runtime ? (details.runtime + ' min') : '';
    const overview = String(details && details.overview || '').trim() || 'Aucun résumé TMDb disponible pour ce flux. Tu peux tout de même relancer le flux, préremplir Stream URL, ou lancer directement la recherche de sous-titres.';
    const genres = normalizeGenres(details && details.genres);
    const langs = Array.isArray(item.langs) && item.langs.length ? item.langs.join(', ') : '—';
    const provider = item.provider || '—';
    const url = String(item.url || '').trim();
    const lastPlayed = formatDate(item.lastPlayedAt || item.updatedAt || item.at);
    const updated = formatDate(item.updatedAt || item.at);

    return {
      backdrop,
      html: `
        <div class="ares-library-modal-grid">
          <div>
            <div class="ares-library-modal-posterwrap">
              <img src="${poster}" alt="Poster ${String(title).replace(/"/g, '&quot;')}">
              ${item.tmdbId ? `<div class="ares-library-modal-posterbadge">TMDb #${item.tmdbId}</div>` : ''}
            </div>
          </div>
          <div>
            <h2 class="ares-library-modal-title">${title}</h2>
            <div class="ares-library-modal-subtitle">${year ? year + ' • ' : ''}${details && details.original_title && details.original_title !== title ? details.original_title : ''}</div>
            <div class="ares-library-modal-badges">
              ${year ? `<span class="ares-library-modal-badge">${year}</span>` : ''}
              ${vote ? `<span class="ares-library-modal-badge">Note ${vote}</span>` : ''}
              ${runtime ? `<span class="ares-library-modal-badge">${runtime}</span>` : ''}
              ${genres ? `<span class="ares-library-modal-badge">${genres}</span>` : ''}
              ${provider && provider !== '—' ? `<span class="ares-library-modal-badge">${provider}</span>` : ''}
            </div>
            <p class="ares-library-modal-overview">${overview.replace(/</g,'&lt;')}</p>
            <div class="ares-library-modal-meta">
              <div class="ares-library-modal-meta-card"><div class="ares-library-modal-meta-label">Langues mémorisées</div><div class="ares-library-modal-meta-value">${langs}</div></div>
              <div class="ares-library-modal-meta-card"><div class="ares-library-modal-meta-label">Dernière lecture</div><div class="ares-library-modal-meta-value">${lastPlayed || '—'}</div></div>
              <div class="ares-library-modal-meta-card"><div class="ares-library-modal-meta-label">Mémoire mise à jour</div><div class="ares-library-modal-meta-value">${updated || '—'}</div></div>
              <div class="ares-library-modal-meta-card"><div class="ares-library-modal-meta-label">Identifiant</div><div class="ares-library-modal-meta-value">${item.tmdbId ? ('TMDb #' + item.tmdbId) : 'Aucun TMDb mémorisé'}</div></div>
            </div>
            <div class="ares-library-modal-url">${url || 'URL non disponible'}</div>
            <div class="ares-library-modal-actions">
              <button type="button" class="primary" data-action="relaunch">Relancer</button>
              <button type="button" data-action="prefill">Préremplir</button>
              <button type="button" data-action="subs">Sous-titres</button>
  ...
  async function showLibraryCardDetails(card){
    const item = getCardItem(card);
    if (!item) return;
    const modal = ensureModal();
    const inner = modal.querySelector('.ares-library-modal-inner');
    const bg = modal.querySelector('.ares-library-modal-backdrop img');
    if (inner) inner.innerHTML = '<div class="ares-library-modal-loading">Chargement de la fiche visuelle…</div>';
    if (bg) bg.src = (card.querySelector('.ares-library-poster') && card.querySelector('.ares-library-poster').src) || FALLBACK_POSTER;
    openModal();

    let details = null;
    try { details = await getTmdbDetailsForItem(item); } catch {}
    const rendered = renderDetailHtml(item, details, card);
    if (bg) bg.src = rendered.backdrop || FALLBACK_POSTER;
    if (inner) inner.innerHTML = rendered.html;

    const actionWrap = inner.querySelector('.ares-library-modal-actions');
    if (actionWrap) {
      actionWrap.addEventListener('click', (e) => {
        const btn = e.target.closest('button[data-action]');
        if (!btn) return;
        const action = btn.getAttribute('data-action');
        if (action === 'relaunch') clickCardAction(card, 'Relancer');
        else if (action === 'prefill') clickCardAction(card, 'Préremplir');
        else if (action === 'subs') clickCardAction(card, 'Sous-titres');
        else if (action === 'remove') {
          clickCardAction(card, 'Supprimer');
          closeModal();
          return;
        }
        if (action !== 'remove') closeModal();
      }, { once: true });
    }
  }
  ...
  function enhanceLibraryCards(root){
    ...
    const cards = Array.from(scope.querySelectorAll('#directMemoryLibraryList .subsearch-result-card'));
    cards.forEach((card) => {
      if (card.dataset.detailEnhanceReady === '1') return;
      card.dataset.detailEnhanceReady = '1';

      const actions = Array.from(card.children).find((el) => el.tagName === 'DIV' && Array.from(el.querySelectorAll('button')).length >= 2);
      if (actions && !actions.querySelector('.ares-library-fiche-btn')) {
        const btn = document.createElement('button');
        btn.type = 'button';
        btn.className = 'subsearch-mini-btn ares-library-fiche-btn';
        btn.textContent = 'Voir fiche';
        btn.addEventListener('click', (e) => {
          e.preventDefault();
          e.stopPropagation();
          showLibraryCardDetails(card);
        });
        actions.insertBefore(btn, actions.firstChild);
      }

      const title = card.querySelector('.subsearch-result-title');
      if (title && !title.dataset.detailBound) {
        title.dataset.detailBound = '1';
        title.style.cursor = 'pointer';
        title.addEventListener('click', () => showLibraryCardDetails(card));
      }
      const poster = card.querySelector('.ares-library-poster');
      if (poster && !poster.dataset.detailBound) {
        poster.dataset.detailBound = '1';
        poster.style.cursor = 'pointer';
        poster.addEventListener('click', () => showLibraryCardDetails(card));
      }
    });
  }
  ...
  function startObservers(){
    const list = document.getElementById('directMemoryLibraryList');
    if (!list) return;
    enhanceLibraryCards(list);
    if (list.dataset.detailObserverReady === '1') return;
    list.dataset.detailObserverReady = '1';
    const mo = new MutationObserver(() => { enhanceLibraryCards(list); });
  ...
  function boot(){
    injectStyles();
    startObservers();
    const panelBtn = document.getElementById('directMemoryHistoryBtn');
    if (panelBtn && !panelBtn.dataset.detailBootBound) {
      panelBtn.dataset.detailBootBound = '1';
      panelBtn.addEventListener('click', () => {
        setTimeout(() => {
          startObservers();
          enhanceLibraryCards(document.getElementById('directMemoryLibraryList') || document);
        }, 0);
      });
    }
  }
  ...
  window.__aresShowLibraryCardDetails = showLibraryCardDetails;
})();


/* ===== Ares direct memory library collection grid ===== */
(function(){
  ...
  function injectStyles(){
    ...
    style.id = 'aresLibraryCollectionStyles';
    style.textContent = `
      #directMemoryHistoryPanel.ares-library-grid-mode{
        background:linear-gradient(180deg, rgba(8,11,18,.68), rgba(6,10,17,.56));
        border-color:rgba(0,229,255,.12);
      }
      #directMemoryLibraryToolbar.ares-library-collection-toolbar{
        grid-template-columns:minmax(0,1fr) minmax(170px,220px) auto;
        align-items:center;
      }
      .ares-library-view-toggle{
        display:inline-flex; gap:6px; justify-content:flex-end; flex-wrap:wrap;
      }
      .ares-library-view-toggle button{
        min-height:38px; padding:0 12px; border-radius:12px;
        border:1px solid rgba(255,145,0,.18);
        background:rgba(255,255,255,.05); color:#eef6ff; cursor:pointer;
      }
      .ares-library-view-toggle button.active{
        background:rgba(255,145,0,.18);
        box-shadow:0 0 0 1px rgba(255,145,0,.18) inset;
      }
      #directMemoryLibraryList.ares-library-grid{
        display:grid !important;
        grid-template-columns:repeat(auto-fill, minmax(180px, 1fr)) !important;
        gap:14px !important;
      }
      #directMemoryLibraryList.ares-library-list{
        display:grid !important;
        grid-template-columns:1fr !important;
        gap:10px !important;
      }
      .ares-library-grid-card{
        position:relative;
        ...
        background:linear-gradient(180deg, rgba(12,18,28,.96), rgba(8,12,20,.98));
        box-shadow:0 14px 34px rgba(0,0,0,.28);
        transition:transform .18s ease, box-shadow .18s ease, border-color .18s ease;
      }
      .ares-library-grid-card:hover{
        transform:translateY(-2px);
        box-shadow:0 20px 40px rgba(0,0,0,.36);
        border-color:rgba(255,145,0,.28);
      }
      .ares-library-grid-card .ares-library-poster-wrap{
        width:100% !important;
        min-width:0 !important;
        align-self:auto !important;
        aspect-ratio:2/3;
      }
      .ares-library-grid-card .ares-library-poster{
        width:100% !important;
      ...
      .ares-library-grid-card .ares-library-body{
        position:absolute;
        inset:auto 0 0 0;
        z-index:2;
        padding:12px;
        gap:8px !important;
        background:linear-gradient(180deg, rgba(5,8,14,0) 0%, rgba(5,8,14,.38) 10%, rgba(5,8,14,.92) 38%, rgba(5,8,14,.98) 100%);
      }
      .ares-library-grid-card::after{
        content:'';
        position:absolute; inset:0;
        background:linear-gradient(180deg, rgba(0,0,0,0) 20%, rgba(5,8,14,.18) 42%, rgba(5,8,14,.82) 78%, rgba(5,8,14,.96) 100%);
        pointer-events:none; z-index:1;
      }
      .ares-library-grid-card .subsearch-result-title{
        color:#fff;
        font-size:15px;
        line-height:1.3;
        font-weight:800;
      ...
      .ares-library-grid-card .subsearch-result-title:hover{
        color:#ffddb2;
      }
      .ares-library-grid-card .subsearch-result-meta{
        color:#d0def6;
        font-size:12px;
        line-height:1.4;
        text-shadow:0 2px 8px rgba(0,0,0,.34);
      }
      .ares-library-grid-card .subsearch-result-meta:last-of-type{
        color:#9fb4d7;
        font-size:11px;
      }
      .ares-library-grid-card .ares-library-actions{
        display:grid !important;
        grid-template-columns:repeat(2, minmax(0,1fr));
        gap:8px;
        margin-top:2px !important;
      }
      .ares-library-grid-card .subsearch-mini-btn{
        min-height:34px;
        border-radius:10px;
        background:rgba(255,255,255,.08);
        backdrop-filter:blur(4px);
      }
      .ares-library-grid-card .ares-library-fiche-btn{
        grid-column:span 2;
      }
      .ares-library-grid-card .ares-library-tmdb-badge{
        top:10px !important;
        right:10px !important;
        bottom:auto !important;
      }

      .ares-library-list-card{
        min-height:158px !important;
      }
      .ares-library-list-card .ares-library-body{
        position:static;
        padding:0;
        background:none;
      }
  ...
  function getViewMode(){
    try {
      return localStorage.getItem('tronAres.directLibraryViewMode') || 'grid';
    } catch {
      return 'grid';
    }
  }
  ...
  function setViewMode(mode){
    ...
    try { localStorage.setItem('tronAres.directLibraryViewMode', next); } catch {}
    applyViewMode();
  }

  function ensureToolbar(){
    injectStyles();
    const toolbar = document.getElementById('directMemoryLibraryToolbar');
    if (!toolbar) return;
    toolbar.classList.add('ares-library-collection-toolbar');
    let wrap = document.getElementById('directMemoryLibraryViewToggle');
    if (!wrap) {
      wrap = document.createElement('div');
      wrap.id = 'directMemoryLibraryViewToggle';
      wrap.className = 'ares-library-view-toggle';
      const gridBtn = document.createElement('button');
      gridBtn.type = 'button';
      gridBtn.dataset.view = 'grid';
      gridBtn.textContent = 'Vue grille';
      gridBtn.addEventListener('click', () => setViewMode('grid'));
      const listBtn = document.createElement('button');
      listBtn.type = 'button';
      listBtn.dataset.view = 'list';
      listBtn.textContent = 'Vue liste';
      listBtn.addEventListener('click', () => setViewMode('list'));
      wrap.append(gridBtn, listBtn);
  ...
  function upgradeCard(card, mode){
    if (!(card instanceof HTMLElement)) return;
    card.classList.remove('ares-library-grid-card', 'ares-library-list-card');
    const actions = Array.from(card.querySelectorAll('button')).length ? Array.from(card.children).find((el) => el.tagName === 'DIV' && el.querySelector('button')) : null;
    if (actions) actions.classList.add('ares-library-actions');

    const body = card.querySelector('.ares-library-body');
    if (!body) return;

    if (mode === 'list') {
      card.classList.add('ares-library-list-card');
      card.style.display = 'flex';
      card.style.flexDirection = 'row';
      card.style.alignItems = 'stretch';
      card.style.gap = '12px';
    ...
    card.classList.add('ares-library-grid-card');
  ...
  function applyViewMode(){
    ensureToolbar();
    const panel = document.getElementById('directMemoryHistoryPanel');
    const list = document.getElementById('directMemoryLibraryList');
    if (!list) return;
    const mode = getViewMode();
    list.classList.remove('ares-library-grid', 'ares-library-list');
    if (panel) panel.classList.add('ares-library-grid-mode');
    list.classList.add(mode === 'list' ? 'ares-library-list' : 'ares-library-grid');

    Array.from(list.querySelectorAll('.subsearch-result-card')).forEach((card) => upgradeCard(card, mode));

    const wrap = document.getElementById('directMemoryLibraryViewToggle');
    if (wrap) {
      wrap.querySelectorAll('button').forEach((btn) => {
  ...
  document.addEventListener('click', (ev) => {
    ...
    if (target.closest('#directMemoryHistoryBtn') || target.closest('#directMemoryLibrarySort') || target.closest('#directMemoryLibrarySearch')) {
      setTimeout(() => {
        try { ensureToolbar(); } catch {}
        try { applyViewMode(); } catch {}
        try { installObserver(); } catch {}
      }, 0);
    }
  }, true);
  ...
  const origRender = window.__aresRenderDirectMemoryHistoryUi;
  if (typeof origRender === 'function' && !origRender.__aresCollectionWrapped) {
    ...
    wrapped.__aresCollectionWrapped = true;
    window.__aresRenderDirectMemoryHistoryUi = wrapped;
  }
...
/* ===== Ares direct memory library smart rows ===== */
(function(){
  let stylesDone = false;
  let observer = null;
  ...
  function injectStyles(){
    ...
    stylesDone = true;
    const style = document.createElement('style');
    style.id = 'aresLibrarySmartRowsStyles';
    style.textContent = `
      #directMemoryLibraryList.ares-library-smartrows{
      ...
      .ares-library-row-head{
        display:flex;
        align-items:end;
        justify-content:space-between;
        gap:12px;
      }
      .ares-library-row-title{
        font-size:15px;
        font-weight:800;
        color:#fff;
        letter-spacing:.2px;
      }
      .ares-library-row-sub{
        font-size:12px;
        color:#9fb4d7;
      }
      .ares-library-row-track{
        display:grid;
        grid-template-columns:repeat(auto-fill, minmax(180px, 1fr));
        gap:14px;
      }
      .ares-library-row.is-compact .ares-library-row-track{
        grid-template-columns:repeat(auto-fill, minmax(160px, 1fr));
      }
      .ares-library-row-empty{
        color:#9fb4d7;
        font-size:12px;
        padding:10px 12px;
        border:1px dashed rgba(0,229,255,.14);
        border-radius:14px;
        background:rgba(255,255,255,.03);
      }
      .ares-library-smartrows-note{
        color:#89a0c4;
        font-size:12px;
        margin-top:-2px;
      }
  ...
  function getSearchValue(){
    const input = document.getElementById('directMemoryLibrarySearch');
    return String(input && input.value || '').trim();
  }

  function getSortValue(){
    const select = document.getElementById('directMemoryLibrarySort');
    return String(select && select.value || 'recent');
  }

  function isGridMode(){
    const list = document.getElementById('directMemoryLibraryList');
    return !!(list && list.classList.contains('ares-library-grid'));
  }

  function collectCards(){
    const list = document.getElementById('directMemoryLibraryList');
    if (!list) return [];
    return Array.from(list.querySelectorAll(':scope > .subsearch-result-card'));
  }

  function parseCard(card, index){
    const title = (card.querySelector('.subsearch-result-title')?.textContent || '').trim();
    const metas = Array.from(card.querySelectorAll('.subsearch-result-meta')).map((el) => (el.textContent || '').trim()).filter(Boolean);
    const metaText = metas.join(' • ');
    const tmdb = /TMDb\s*#\s*(\d+)/i.exec(metaText);
    const url = metas.find((v) => /^https?:\/\//i.test(v)) || '';
    const hasTmdb = !!tmdb;
    const hasProvider = /opensubtitles|subdl/i.test(metaText);
    const langMatch = metaText.match(/\b([a-z]{2}(?:-[a-z]{2})?)(?:\s*,\s*[a-z]{2}(?:-[a-z]{2})?)+\b/i);
    const langs = langMatch ? langMatch[0].split(',').map((s) => s.trim()).filter(Boolean) : [];
    const lastSeenText = metas.find((v) => /Lu\s+/i.test(v)) || '';
    return {
      card,
      index,
      title,
      ...
      tmdbId: tmdb ? tmdb[1] : '',
      langs,
      multiLang: langs.length >= 2,
      providerKnown: hasProvider,
      recentHint: /à l’instant|il y a\s+\d+\s*(min|h|j)/i.test(lastSeenText)
    };
  }

  function buildRow(title, subtitle, items, compact = false){
    const row = document.createElement('section');
    row.className = 'ares-library-row' + (compact ? ' is-compact' : '');
    const head = document.createElement('div');
    head.className = 'ares-library-row-head';
    const left = document.createElement('div');
    const h = document.createElement('div');
    h.className = 'ares-library-row-title';
    h.textContent = title;
    const sub = document.createElement('div');
    sub.className = 'ares-library-row-sub';
    ...
    if (!items.length) {
      const empty = document.createElement('div');
      empty.className = 'ares-library-row-empty';
      empty.textContent = 'Aucun flux pour cette rangée.';
      row.appendChild(empty);
      return row;
    }

    const track = document.createElement('div');
    track.className = 'ares-library-row-track';
    items.forEach((item) => track.appendChild(item.card));
    row.appendChild(track);
    return row;
  }

  function applySmartRows(){
    injectStyles();
    const list = document.getElementById('directMemoryLibraryList');
    if (!list) return;
    ...
    const modeGrid = isGridMode();
    const baseCards = collectCards();

    list.querySelectorAll(':scope > .ares-library-row, :scope > .ares-library-smartrows-note').forEach((el) => el.remove());
    list.classList.remove('ares-library-smartrows');

    if (!modeGrid || query || sort !== 'recent' || !baseCards.length) return;

    const items = baseCards.map(parseCard);
    const rows = [];

    const recent = items.slice(0, 12);
    const tmdb = items.filter((item) => item.hasTmdb).slice(0, 12);
    const noTmdb = items.filter((item) => !item.hasTmdb).slice(0, 12);
    const multi = items.filter((item) => item.multiLang).slice(0, 12);

    const used = new Set();
    function unique(list){
      return list.filter((item) => {
        if (used.has(item.card)) return false;
        used.add(item.card);
        return true;
      });
    }

    rows.push(buildRow('Récemment lus', 'Les derniers flux repris dans ta bibliothèque.', unique(recent)));
    rows.push(buildRow('Avec TMDb', 'Flux reconnus avec une fiche média propre.', unique(tmdb), true));
    rows.push(buildRow('Sans TMDb', 'Flux encore sans identification TMDb.', unique(noTmdb), true));
    rows.push(buildRow('Multi-langues', 'Flux mémorisés avec plusieurs langues utiles pour les sous-titres.', unique(multi), true));

    const leftovers = items.filter((item) => !used.has(item.card)).slice(0, 24);
    if (leftovers.length) {
      rows.push(buildRow('Autres flux', 'Le reste de la bibliothèque mémorisée.', leftovers, true));
    }

    baseCards.forEach((card) => card.remove());
    list.classList.add('ares-library-smartrows');
    const note = document.createElement('div');
    note.className = 'ares-library-smartrows-note';
    note.textContent = 'Rangées intelligentes actives en vue grille, sans recherche, avec tri “plus récents”.';
  ...
  const origRender = window.__aresRenderDirectMemoryHistoryUi;
  if (typeof origRender === 'function' && !origRender.__aresSmartRowsWrapped) {
    const wrapped = function(){
      const result = origRender.apply(this, arguments);
      setTimeout(() => {
        try { applySmartRows(); } catch {}
    ...
    wrapped.__aresSmartRowsWrapped = true;
    window.__aresRenderDirectMemoryHistoryUi = wrapped;
  }
  ...
  document.addEventListener('click', (ev) => {
    const target = ev.target;
    if (!(target instanceof Element)) return;
    if (target.closest('#directMemoryHistoryBtn') || target.closest('#directMemoryLibraryViewToggle') || target.closest('#directMemoryLibrarySort')) {
      setTimeout(() => {
        try { applySmartRows(); } catch {}
        try { installObserver(); } catch {}
      }, 0);
  ...
  document.addEventListener('input', (ev) => {
    const target = ev.target;
    if (!(target instanceof Element)) return;
    if (target.closest('#directMemoryLibrarySearch')) {
      setTimeout(() => {
        try { applySmartRows(); } catch {}
      }, 0);
    }
...
/* ===== Ares stream URL card pro memory controls ===== */
(function(){
  ...
  function readMemoryItem(url){
    try {
      if (typeof __subtitleRecallForUrl === 'function') return __subtitleRecallForUrl(url) || null;
    } catch {}
    try {
      const raw = localStorage.getItem('tronAres.directSubtitleMemory.v1');
      const obj = raw ? JSON.parse(raw) : null;
      return obj && url ? (obj[url] || null) : null;
    } catch { return null; }
  }

  function getRecognizedLabel(item){
    if (!item) return '';
    const title = String(item.queryLabel || item.title || '').trim();
    const year = String(item.year || '').trim();
    if (title && year && !title.includes(year)) return title + ' (' + year + ')';
    return title || '';
  }

  function ensureProStreamUrlUi(){
    const card = document.querySelector('#streamUrlOverlay .streamurl-card');
    const actions = qs('streamUrlPlayBtn')?.closest('.streamurl-actions') || card?.querySelector('.streamurl-actions');
    const hint = card?.querySelector('.streamurl-hint');
    if (!card || !actions) return null;

    let wrap = qs('streamUrlMemoryProWrap');
    let fileInput = qs('streamUrlMemoryImportInput');
    let importBtn = qs('streamUrlMemoryImportBtn');
    let libraryBtn = qs('streamUrlMemoryLibraryBtn');
    let fillBtn = qs('streamUrlMemoryFillBtn');
    let badge = qs('streamUrlMemoryRecognizedBadge');
    let meta = qs('streamUrlMemoryRecognizedMeta');

    if (!wrap) {
      wrap = document.createElement('div');
      wrap.id = 'streamUrlMemoryProWrap';
      wrap.style.marginTop = '12px';
      wrap.style.display = 'grid';
      wrap.style.gap = '10px';

      const row = document.createElement('div');
      row.className = 'streamurl-actions';
      ...
      importBtn = document.createElement('button');
      importBtn.type = 'button';
      importBtn.id = 'streamUrlMemoryImportBtn';
      importBtn.className = 'btn btn-ghost';
      importBtn.textContent = 'Importer mémoire';

      libraryBtn = document.createElement('button');
      libraryBtn.type = 'button';
      libraryBtn.id = 'streamUrlMemoryLibraryBtn';
      libraryBtn.className = 'btn btn-ghost';
      libraryBtn.textContent = 'Bibliothèque flux';

      fillBtn = document.createElement('button');
      fillBtn.type = 'button';
      fillBtn.id = 'streamUrlMemoryFillBtn';
      fillBtn.className = 'btn btn-accent';
      fillBtn.textContent = 'Préremplir reconnu';
      fillBtn.style.display = 'none';

      row.append(importBtn, libraryBtn, fillBtn);

      const recognized = document.createElement('div');
      recognized.id = 'streamUrlMemoryRecognizedBox';
      recognized.style.display = 'none';
      recognized.style.padding = '10px 12px';
      recognized.style.borderRadius = '14px';
      recognized.style.border = '1px solid rgba(255,145,0,.18)';
      recognized.style.background = 'rgba(7,10,17,.46)';
      recognized.style.boxShadow = '0 8px 22px rgba(0,0,0,.18)';

      badge = document.createElement('div');
      badge.id = 'streamUrlMemoryRecognizedBadge';
      badge.style.fontWeight = '700';
      badge.style.marginBottom = '4px';
      badge.textContent = 'Film reconnu';

      meta = document.createElement('div');
      meta.id = 'streamUrlMemoryRecognizedMeta';
      meta.className = 'streamurl-hint';
      meta.textContent = 'Aucune reconnaissance mémorisée pour cette URL.';

      recognized.append(badge, meta);

      fileInput = document.createElement('input');
      fileInput.type = 'file';
      fileInput.id = 'streamUrlMemoryImportInput';
      fileInput.accept = '.json,application/json';
      fileInput.style.display = 'none';

      wrap.append(row, recognized, fileInput);
      if (hint && hint.parentNode) hint.parentNode.insertBefore(wrap, hint.nextSibling);
    ...
    if (fileInput && !fileInput.dataset.bound) {
      ...
      fileInput.addEventListener('change', async () => {
        ...
        try {
          const text = await file.text();
          if (typeof window.__aresImportDirectMemoryFromText === 'function') {
            window.__aresImportDirectMemoryFromText(text);
          } else {
            throw new Error('Import mémoire indisponible');
          }
          updateRecognizedInfo();
        } catch (e) {
          console.error(e);
          try { setStatus('Import mémoire impossible : ' + (e && e.message ? e.message : String(e))); } catch {}
        } finally {
          try { fileInput.value = ''; } catch {}
        }
      });
    ...
    if (libraryBtn && !libraryBtn.dataset.bound) {
      ...
      libraryBtn.addEventListener('click', () => {
        try {
          const title = String(qs('streamTitleInput')?.value || '').trim();
          if (typeof __subtitleOpenOverlay === 'function') {
            __subtitleOpenOverlay(title);
          } else {
            const subtitleBtn = qs('subtitleTrackBtn');
            subtitleBtn?.click();
          }
          setTimeout(() => {
            try {
              const btn = qs('directMemoryHistoryBtn');
              const panel = qs('directMemoryHistoryPanel');
              if (btn && panel && panel.style.display === 'none') btn.click();
              else if (typeof window.__aresRenderDirectMemoryHistoryUi === 'function') window.__aresRenderDirectMemoryHistoryUi();
            } catch {}
          }, 60);
        } catch (e) {
          console.error(e);
    ...
    if (fillBtn && !fillBtn.dataset.bound) {
      ...
      fillBtn.addEventListener('click', () => {
        const url = normalizeUrl(qs('streamUrlInput')?.value || '');
        const remembered = readMemoryItem(url);
        const label = getRecognizedLabel(remembered);
        if (!remembered || !label) {
          try { setStatus('Aucun titre mémorisé pour ce flux'); } catch {}
          updateRecognizedInfo();
          return;
        }
        const titleInput = qs('streamTitleInput');
        if (titleInput) titleInput.value = label;
        try { setStatus('Titre mémorisé prérempli pour ce flux'); } catch {}
        updateRecognizedInfo();
      });
    }

    return { wrap, importBtn, libraryBtn, fillBtn, badge, meta };
  }

  function updateRecognizedInfo(){
    const ui = ensureProStreamUrlUi();
    if (!ui) return;
    const box = qs('streamUrlMemoryRecognizedBox');
    const fillBtn = ui.fillBtn || qs('streamUrlMemoryFillBtn');
    const badge = ui.badge || qs('streamUrlMemoryRecognizedBadge');
    const meta = ui.meta || qs('streamUrlMemoryRecognizedMeta');
    const url = normalizeUrl(qs('streamUrlInput')?.value || '');
    const remembered = readMemoryItem(url);
    const label = getRecognizedLabel(remembered);

    if (!box || !badge || !meta || !fillBtn) return;
    ...
    if (!remembered || !label) {
      ...
      fillBtn.style.display = 'none';
      badge.textContent = 'Film reconnu';
      meta.textContent = 'Aucune fiche mémorisée pour cette URL. Tu peux importer une mémoire ou ouvrir la bibliothèque.';
      return;
    }

    box.style.display = '';
    fillBtn.style.display = '';
    badge.textContent = 'Film reconnu : ' + label;
    const bits = [];
    if (remembered.tmdbId) bits.push('TMDb #' + remembered.tmdbId);
    if (remembered.provider) bits.push('Provider ' + remembered.provider);
    if (Array.isArray(remembered.langs) && remembered.langs.length) bits.push('Langues ' + remembered.langs.join(', '));
    if (remembered.updatedAt) {
      try { bits.push('Mis à jour ' + new Date(Number(remembered.updatedAt)).toLocaleString('fr-FR')); } catch {}
    }
    meta.textContent = bits.join(' • ') || 'Identité mémorisée prête à être réutilisée.';
  }

  const origOpenStreamUrlPanel = typeof openStreamUrlPanel === 'function' ? openStreamUrlPanel : null;
  if (origOpenStreamUrlPanel && !origOpenStreamUrlPanel.__aresProWrapped) {
    const wrapped = function(prefillFromQuery = true){
      const result = origOpenStreamUrlPanel.call(this, prefillFromQuery);
      try { ensureProStreamUrlUi(); } catch {}
      try { updateRecognizedInfo(); } catch {}
      return result;
    };
    wrapped.__aresProWrapped = true;
    openStreamUrlPanel = wrapped;
  }

  const streamUrlInput = qs('streamUrlInput');
  const streamTitleInput = qs('streamTitleInput');
  streamUrlInput?.addEventListener('input', updateRecognizedInfo);
  streamUrlInput?.addEventListener('change', updateRecognizedInfo);
  streamUrlInput?.addEventListener('blur', updateRecognizedInfo);
  streamTitleInput?.addEventListener('input', updateRecognizedInfo);
  window.addEventListener('storage', (ev) => {
    if (!ev || ev.key === 'tronAres.directSubtitleMemory.v1' || ev.key === 'tronAres.directSubtitleHistory.v1') updateRecognizedInfo();
  });
