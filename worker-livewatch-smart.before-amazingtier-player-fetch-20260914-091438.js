...
function hasStableIframeFallback(channel) {
  const fallback = channel?.manualSources?.html;
  if (!fallback || fallback.kind !== "iframe" || !fallback.iframeUrl) return false;
  return Object.values(channel.manualSources || {}).some((source) =>
    source !== fallback &&
    (source?.kind === "clouding" || source?.kind === "lovetier") &&
    Boolean(source.cloudingChannel || source.lovetierChannel)
  );
}

function isRedundantHtmlFallback(channel, key) {
  return String(key || "").toLowerCase() === "html" && hasStableIframeFallback(channel);
}

function manualSourceNames(channel) {
  const automatic = new Set(Object.keys(channel.sources || {}));
  return Object.keys(allSources(channel))
    .filter((key) => !automatic.has(key))
    .filter((key) => !isRedundantHtmlFallback(channel, key));
}
...
function isAllowedLovetierUrl(url) {
  ...
  return (url.origin === LOVETIER_ORIGIN || isBluetierOrigin(url)) &&
    Array.from(paths).some((path) => url.pathname.toLowerCase().startsWith(path)) &&
    !url.username &&
    !url.password;
}

function isBluetierOrigin(url) {
...
async function resolveAmazingTierSource(channelKey, sourceName, source) {
  ...
  // The player page can be blocked for Cloudflare Worker requests. The
  // documented refresh endpoint returns the same short-lived token directly.
  const tokenResponse = await fetchWithTimeout(new URL("/api/refresh_token.php", AMAZINGTIER_PLAYER_ORIGIN), {
    method: "POST",
    headers: {
  ...
  if (!tokenResponse.ok || tokenData?.success !== true || !token) {
    throw new Error(`amazingtier token ${tokenResponse.status}`);
  }

  const upstreamUrl = new URL(`/${encodeURIComponent(channel)}/index.m3u8`, AMAZINGTIER_ORIGIN);
...
function playerPage(origin, channelKey, channel) {
  ...
  const htmlFallback = channel.manualSources?.html;
  const hasIframeFallback = hasStableIframeFallback(channel);
  ...
  for (const [key, source] of Object.entries(sources)) {
    ...
    if (source.kind === "clouding" || source.kind === "lovetier") {
      stableHlsSourceKeys.push(key);
      // The named stable source owns the Yoyo slot. When a matching HTML
      // wrapper exists, use it here so its token watchdog and reload logic are
      // not exposed as a second duplicate button.
      if (hasIframeFallback && htmlFallback?.iframeUrl) {
        sourceRedirects[key] = htmlFallback.iframeUrl;
      } else if (source.kind === "clouding") {
        // AmazingTier refuses token renewal from Cloudflare egress, while its
        // own player can renew the same token in the viewer's browser.
  ...
  const sourceButtons = Object.entries(sources)
    .filter(([key]) => !isRedundantHtmlFallback(channel, key))
    .map(([key, source]) => {
...
<head>
  ...
  <style>
    * { box-sizing: border-box; }
    ...
    #sourceFrame { position: absolute; inset: 0; width: 100%; height: 100%; border: 0; background: #000; display: block; }
    #sourceFrame[hidden] { display: none; }
...
<body>
  <main>
    ...
    <iframe id="sourceFrame" title="Lecteur de secours" allow="autoplay; fullscreen; picture-in-picture; encrypted-media" referrerpolicy="no-referrer" hidden></iframe>
  ...
  <script>
    ...
    function showVideoSurface() {
      if (sourceFrame) {
        ...
        if (sourceFrame.src && sourceFrame.src !== 'about:blank') sourceFrame.src = 'about:blank';
      }
      video.style.visibility = 'visible';
    }
    function showRedirectSurface(key, reason) {
      if (!sourceFrame || !SOURCE_REDIRECTS[key]) return false;
      ...
      sourceFrame.title = SOURCE_LABELS[key] || 'Lecteur de secours';
      sourceFrame.hidden = false;
      sourceFrame.src = SOURCE_REDIRECTS[key];
      appendLog('source-frame-start', { key: key, url: SOURCE_REDIRECTS[key], reason: reason || 'redirect' });
      setLoadStatus('ok', 'Lecteur de secours actif', SOURCE_LABELS[key] || key);
      return true;
    }
    ...
    function loadSourceKey(key, label, sequence, index, reason) {
      ...
      if (SOURCE_REDIRECTS[key]) {
        ...
        showRedirectSurface(key, reason || 'manual');
        return;
      }
