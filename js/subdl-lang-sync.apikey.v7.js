...
(function () {
  ...
  // -----------------------------------------------------
  // OpenSubtitles settings gear visibility (fallback)
  // -----------------------------------------------------
  function setOsSettingsButtonsVisible(visible){
    try{
      var ov = $("subtitleSearchOverlay");
      if(!ov || !ov.querySelector) return;
      var gear = ov.querySelector('button.subsearch-mini-btn[title="Réglages OpenSubtitles"]');
      if(gear) gear.style.display = visible ? "" : "none";
    }catch(e){}
  }

  function wireOsSettingsToggle(){
    if(wireOsSettingsToggle._done) return;
    wireOsSettingsToggle._done = true;

    var cb = $("toggleOsSettingsBtnCheckbox");
    if(!cb) return;

    // Apply now
    setOsSettingsButtonsVisible(!!cb.checked);
    ...
    // If the gear button is injected later (by js/tron-ares.js), watch the overlay
    var ov = $("subtitleSearchOverlay");
    ...
    mo.observe(ov, { attributes:true, childList:true, subtree:true, attributeFilter:["class"] });
  }
