...
function __subtitleGuessTitleFromUrl(url) {
  try {
    ...
    const last = (rawPath.split('/').filter(Boolean).pop() || '').trim();
    if (!last) return '';
    return __subtitleNormalizeSearchLabel(last);
  } catch {
    const fallback = String(url || '').split('/').pop() || '';
    return __subtitleNormalizeSearchLabel(fallback);
  }
}
...
function __subtitleOpenOverlay(prefillTitle = '') {
  ...
  const provider = subtitleSearchProviderSelect ? subtitleSearchProviderSelect.value : 'opensubtitles';
  const langs = (subtitleSearchLangInput?.value || '').trim();
  const normalizedTitle = __subtitleNormalizeSearchLabel(prefillTitle || '');
